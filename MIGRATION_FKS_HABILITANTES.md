...
**⚠️ CRÍTICO — el SQL Editor de Supabase NO mantiene transacciones entre ejecuciones separadas** (aprendizaje operacional A2 prod 2026-08-14, 6ª instancia del corolario P8 en CLAUDE.md):

Cada botón "Run" abre su propia transacción implícita y hace COMMIT o ROLLBACK al terminar la corrida. Un `BEGIN` en una corrida y un `COMMIT` en otra NO forman una única transacción — el `BEGIN` de la primera corrida cierra en ROLLBACK al finalizar la ejecución de esa corrida, y el `COMMIT` de la segunda corrida no tiene transacción abierta que persistir. **El `RETURNING` de la primera corrida devuelve las filas correctamente (parece exitoso), pero la transacción hace ROLLBACK silencioso al terminar la ejecución**. Output correcto, efecto ausente — patrón P8 puro.

**Regla operativa**: `BEGIN;` + `DELETE ... RETURNING ...;` + `COMMIT;` deben ir **en un único bloque enviado con un único click de Run**. Nunca separar corridas. Aldo puede inspeccionar el `RETURNING` embebido en el output de la corrida única — si algo se ve raro, la próxima corrida es `ROLLBACK;` (pero solo funciona si `BEGIN;` está aún abierto, lo cual **no es el caso en SQL Editor** — así que en la práctica: si el resultado del RETURNING no matchea el dry-run, no ejecutar el bloque completo por segunda vez; en un cliente psql o Studio real sí funcionaría, en SQL Editor Supabase no).

**Alternativa segura para dudas mid-flow**: correr primero el bloque **sin** `COMMIT` — solo `BEGIN;` + `DELETE ... RETURNING ...;` (sin línea COMMIT). La corrida terminará en ROLLBACK automático porque el editor no persiste transacciones abiertas, y el `RETURNING` se ve como "modo dry-run destructivo". Si el output matchea lo esperado, correr un segundo bloque completo con `BEGIN;` + `DELETE;` + `COMMIT;` en una sola corrida esta vez para persistir.

Corrida A2 prod (2026-08-14, Aldo): primer intento fue `BEGIN` en corrida 1 + `DELETE ... RETURNING` (9 filas ejemplo devueltas correctamente) en corrida 2 + `COMMIT` en corrida 3 → rollback silente entre 1 y 2, el DELETE fue no-op efectivo. Se detectó al re-verificar con `SELECT COUNT(*) WHERE es_ejemplo=true` y ver que seguían las 9 filas. Segundo intento con `BEGIN; DELETE ... RETURNING ...; COMMIT;` en un solo bloque → persistió correctamente (`ejemplos_restantes: 0`, `proveedores: 20→11`).
