...
## B3 discovered fixmes (2026-09-24 tras corolario P8 13ª ampliado)

Al agregar 5 dirs faltantes al workflow rápido (B3 fix commit `a69aecd`), 9 specs previamente invisibles corrieron por primera vez en CI. **8 de 9 pasaron verde**; 1 quedó con fail que requiere sprint dedicado:

| Etiqueta | Spec | Motivo (1 línea) | Categoría P11 | Plan Tramo 2 |
|---|---|---|---|---|
| `[b3-discovered-2026-09-25]` | `e2e/specs/producto-1/s1-badge-reserva-online.spec.ts:74` | Card recién creada NO aparece en /explorar ni con filtro `?categoria=cuidado&comuna=Providencia` (fix intentado `50cf8a0`). Query params ignorados por /explorar O delay INSERT→RPC O filtro adicional del RPC (activo/aprobado/geo) | Bug del spec MAYOR que <30 min (fix intentado no aterrizó) | Sprint dedicado post-viaje: (1) inspeccionar `/explorar.tsx` para ver si acepta `?categoria` y `?comuna` como query params; (2) medir delay INSERT servicio → primer read del RPC; (3) verificar predicados del RPC `buscar_servicios` (activo, estado del proveedor, `agendamiento_habilitado`, etc). Post-diagnóstico: unfixme + fix del spec real (probable: `page.goto` directo al `/servicio/[id]` bypassing /explorar; el spec verificaría el badge en la ficha en vez de en la card del listado — cambio de scope pero preserva la assertion funcional del PR1 original) |

**Cero data-cleanup requerido** (no era 4to residuo). **Cero PR aparte** por este fixme — el sprint dedicado post-viaje se ocupa. **PR #85 sigue hacia verde** con el `test.fixme` explícito.

**Regla nueva aterrizada por este descubrimiento**: skip explícito con etiqueta + ticket sí; skip silencioso NUNCA MÁS. Aplicable a partir de 2026-09-24. Documentada en cue-1-fix-b2-reporte-espejo.md sección "Plan operativo P11 para specs recién incluidos por B3".
...
### Episodios registrados como evidencia empírica de la hipótesis "flakiness cross-PR"

| # | fecha | PR | run_id FAIL | fails | run_id VERDE (misma rama, commits posteriores diagnóstico) | delta commits | interpretación |
|---|---|---|---|---|---|---|---|
| 1 | 2026-09-24 | #90 chat-open-error | `36040364041` | 8 fails: 1 propio del PR (`chat-open-error/regresion.spec.ts:57` caso a, timeout URL) + 7 pre-existentes: 4× `pan-1/def3-bell-user-context.spec.ts` (T1/T2/T3/T4), 1× `launch-l1/toast-dup-regresion.spec.ts:73`, 2× `tipo-b/lote-4-admin-notificaciones.spec.ts` (1, 2) | `36045751347` (post `ca78fe1`, mismo día ~1h después): **5/5 pass**, incluyendo los 7 pre-existentes | 1 commit de diagnóstico ampliado del caso a (`ca78fe1`) — cero cambio productivo de la superficie de los 7 pre-existentes | Los 7 pre-existentes pasaban en runs verdes previos del workflow (`sentry-boundary @ a99fe8c/bc367ad/d41ce1a`, `cue-1-fix @ a469b5a`, `button-unif-doc @ 7d39d4a`) → **no son deuda histórica, es race del ambiente**. Cross-PR / cold-start / saturación Supabase transitoria. Consistente con la hipótesis A del CI-CONCURRENCY. El caso (a) propio fue bug de spec (selector `getByRole.first()` clickeó botón disabled sin efecto); el fix `getByLabel + toBeEnabled` cerró ese vector. |

**Interpretación acumulada**: el patrón "N fails simultáneos donde los 7 pre-existentes pasaban en runs verdes anteriores" es la evidencia empírica que motivó al PO a rechazar "fixme en lote" y forzar la ENMIENDA P11 en CLAUDE.md (2026-09-24). Cada instancia futura del patrón agregar una fila a esta tabla + citar el run_id FAIL + run_id VERDE post-diagnóstico como evidencia de que aplicar CI-CONCURRENCY estructural (opción A del kickoff domingo) es la solución correcta.
