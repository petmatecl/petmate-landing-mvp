...
| 8 | **incidente-usuario-fix** (rename `hf-usuario-fix`) · `pages/mensajes.tsx` defaultReturn tutor `/usuario` → `/mis-reservas` + copy dinámico + guard `if (!categoria) return <NotFoundContent />` en `pages/[categoria]/index.tsx` + spec regresión 3 casos | #91 | pendiente merge sábado tras 5/5 verde (guard token loud ampliado en `authenticate.ts`) | — |
...
## 3. Cola de PRs abiertos

| # | PR | rama | contenido (1 línea) | checks | dependencias | qué hacer |
|---|---|---|---|---|---|---|
| 1 | **#91** | `hf-usuario-fix` | Rename de #89 · fix `pages/mensajes.tsx` defaultReturn tutor + guard `[categoria]` con `NotFoundContent` + `authenticate.ts` guard token loud (3 iteraciones hasta estabilizarse) | pendiente 5/5 verde (monitor `bqpxk0sea` post-`d115c69`) | Ninguna | **GO ya dado — merge autónomo al 5/5** + rebase de la cola. QA PO en prod (después): (a) "Volver a mis reservas" desde el chat aterriza en `/mis-reservas` sin error; (b) categoría inexistente por URL (ej. `/usuario`) muestra el 404 canónico de la app. La QA "chat existente al primer clic" es del sprint #90 (chat-open-error) — se hace en la misma pasada de smoke |
| 2 | **#85** | `cue-1-fix` | fase A reproducción diagnóstica + fix stale closure watchdog B2 + F4 doble + B3 CI-SPEC-COUNT | 5/5 verde (post-rebase `a469b5a`) | Ninguna | **Mergear sábado 2026-09-27** con QA PO (2 tabs + logout + cero events nuevos con release). No tocar hasta el sábado |
| 3 | **#87** | `button-unif-doc` | Kickoff sprint BUTTON-UNIF (Tramo 2, sin merge) · docs de decisiones de UI para el desfile visual | UNKNOWN (docs-only sin código productivo) | Ninguna | **Mantener abierto durante el viaje**. Sprint visual de botones = alcance Tramo 2 |
| 4 | **#86** | `supajs-upgrade` | Experimento B1 `@supabase/supabase-js ~2.84 → ~2.117` + remove noOpLock (sin merge, P8 falseó causa CUE-1) | UNKNOWN | Ninguna | **Mantener abierto durante el viaje**. Falseó la hipótesis SDK bug — mantenido como referencia para bump preventivo al regreso. No es urgente |
| 5 | **#84** | `del-cuenta-docs` | Kickoff DEL-CUENTA-LEY + decisiones PO + copy + preguntas asesor (Tramo 2, sin merge) | UNKNOWN | Espera respuestas del asesor legal | **Mantener abierto durante el viaje**. Ejecución Tramo 2 tras SENTRY-TOAST |
| 6 | **#77** | `auth-mail-phish` | 6 plantillas Auth ES + NOTIF-FROM-HARDCODE + DMARC en BACKLOG | UNKNOWN (rebase reciente) | Ninguna | **Mantener abierto durante el viaje**. Merge en Tramo 2 cuando el PO revise las plantillas |
