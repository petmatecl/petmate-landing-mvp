...
|---|---|---|---|---|---|---|
| 1 | **#91** | `hf-usuario-fix` | Rename de #89 · fix `pages/mensajes.tsx` defaultReturn tutor + guard `[categoria]` con `NotFoundContent` + `authenticate.ts` guard token loud | pendiente (monitor `b6tv5r561` armado post-`ebc6913`) | Ninguna | **Mergear antes del viaje** con QA PO tras 5/5 verde. BLOQUEA — tutor volviendo del chat ve pantalla de error |
