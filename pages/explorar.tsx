    ...
    useEffect(() => {
        async function fetchPetmatesAndProfile() {
            ...
            try {
                ...
                // 2. Fetch Petmates using Secure API
                const params = new URLSearchParams();
                if (filters.petType && filters.petType !== 'any') params.append('query', filters.petType); // Basic text match support via query if needed, or update API to support filters. 
                // Note: The API currently supports 'query' param for text search.
                // For MVP: We will assume the API returns all approved sitters if no query, and we filter client side, OR we pass query.
                // Re-reading API code: It returns all approved sitters if query is empty.

                // Let's pass the search term if it exists in filters or just fetch all allowed.
                // The current API implementation supports 'query' which matches name/comuna.
                // Complex filters (dates/services) are client-side filtered in this file later?
                // Looking at line 125+, there was client side filtering?
                // Actually the RPC handled it server side. 
                // P0 Fix: Secure the DATA first. Filtering performance is secondary for MVP unless >1000 users.
                // We will fetch ALL public (approved) sitters and client-filter for now to respect the P0 data safety requirement.

                const response = await fetch('/api/sitters/search');
                if (!response.ok) throw new Error('Search failed');

                const { data: apiData } = await response.json();

                // Client-side filtering to maintain feature parity (until API gets complex filters)
                // Filter by Service Type
                let filtered = apiData || [];

                if (filters.serviceType && filters.serviceType !== 'all') {
                    // Check specific service flags
                    if ((filters.serviceType as string) === 'hospedaje') {
                        filtered = filtered.filter((p: any) => p.servicio_en_casa);
                    } else if ((filters.serviceType as string) === 'domicilio') {
                        filtered = filtered.filter((p: any) => p.servicio_a_domicilio);
                    }
                }

                // Filter by Pet Type (simple approximation if fields exist, otherwise skip)
                if (filters.petType && filters.petType !== 'any') {
                    if ((filters.petType as string) === 'perro') {
                        filtered = filtered.filter((p: any) => p.cuida_perros);
                    } else if ((filters.petType as string) === 'gato') {
                        filtered = filtered.filter((p: any) => p.cuida_gatos);
                    }
                }

                setPetmates(filtered);
                const error = null; // Clear error
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-20">
                ...
                {loading || checkingProfile ? (
                    ...
                    <>
                        {/* Soft Banner for Incomplete Profile */}
                        {isAuthenticated && clientMissingFields.length > 0 && (
                            <div className="mb-6 bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-4 items-start animate-in fade-in slide-in-from-top-4">
                                <span className="text-amber-500 text-xl">📝</span>
                                <div className="flex-1">
                                    <h3 className="text-sm font-bold text-amber-800">Completa tu perfil para contactar</h3>
                                    <p className="text-xs text-amber-700 mt-1">
                                        Puedes explorar libremente, pero necesitarás completar tu información ({clientMissingFields.join(", ")}) antes de enviar solicitudes.
                                    </p>
                                </div>
                                <Link
                                    href="/usuario"
                                    className="px-4 py-2 bg-amber-100 text-amber-800 text-xs font-bold rounded-lg hover:bg-amber-200 transition-colors whitespace-nowrap"
                                >
                                    Ir a mi Perfil
                                </Link>
                            </div>
