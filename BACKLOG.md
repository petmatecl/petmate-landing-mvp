...
- **Copy de emails de confirmación de reserva por horas — mostrar rango completo** (pedido de PO, 2026-07-28). Hoy los emails de confirmación F1 (reserva por bloque horario, `duracion_min NOT NULL`) muestran la hora de inicio + la duración por separado (ej. "14:00" + "1 hora"). El PO pide un solo bloque legible con el rango completo: **"de 14:00 a 15:00 · 1 hora"**. Aplicable a todos los templates de confirmación donde aparece la hora + duración: `ReservaConfirmadaTutorEmail`, `AgendamientoTutorEmail` (branch `estado='confirmada'`), `AgendamientoProveedorEmail` (branch `esConfirmadaAuto`), y el equivalente de proveedor cuando exista. El helper de formato puede vivir en `lib/formatFecha.ts` (patrón espejo del ya existente `formatRangoNoches` para F2). Solo aplica a F1 con `duracionLabel` populado (F2 usa rango de noches, ya cubre bien). Cambio de copy + render-diff no-regresión, sprint chico post-launch.
