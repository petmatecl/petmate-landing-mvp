...
- **[abierto, PRIORIDAD ALTA (subida de media 2026-09-04) — bug PREEXISTENTE GENERAL de toda la app] Toast duplicado en TODAS las acciones del sitio, no solo un toggle**. Detectado inicialmente en smoke role-degradation (2026-09-04) al pausar/activar servicio del panel proveedor: **2 toasts distintos por una sola acción** (uno verde con check verde, uno blanco con check negro). **ACTUALIZACIÓN CRÍTICA sprint carto-key smoke (2026-09-04)**: mismo patrón al guardar el perfil desde el dashboard — "Perfil actualizado correctamente" también sale dos veces con las mismas dos variantes. **NO es un caller que llama `toast()` dos veces** (hipótesis original refutada); es algo común a todos los toasts del sitio. **Hipótesis viva refinada**: probablemente hay **dos `<Toaster>` montados en el árbol** (uno de sonner "canónico" en `_app.tsx:63` + otro montado inadvertidamente en algún componente hijo — el de `ServiceFormModal.tsx:2578` que se conocía como "duplicado a limpiar" en la entrada de homologación toasts del PDPO probablemente lo explica). Cada Toaster renderiza cada toast disparado → dos elementos visuales por invocación. Método verificable al abrir: grep `<Toaster` en `**/*.tsx` = espera 1 solo match en `_app.tsx`; si aparece otro, ese es el culpable. Sprint chico (~15-30 min): quitar el Toaster duplicado + smoke confirmando 1 toast por acción en perfil + toggle + review + otras. Prioridad **subida a alta** porque afecta a **toda la app**, no una pantalla. Método de descarte de falsa alarma "regresión del sprint" documentado en `ACTA_ROLE_DEGRADATION.md §10` como caso replicable.
