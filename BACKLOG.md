...
- **Instrumentar `/api/cron/recordatorio-reserva` para diagnóstico de drift de idempotencia** (observado en R6). La suite R6 originalmente ejecutaba la 2ª corrida como `real` (no dryRun) y verificaba con un ancla temporal `preRun2Ms` que ninguna marca fuera reescrita durante esa 2ª corrida. Observación consistente: la marca `recordatorio_tutor_enviado_at` del agendamiento F1 test aparece con timestamp ~200-400ms tras `preRun2Ms` — solo la del tutor, solo del F1, en ambos intentos (retry #1 también falla). Serialización cross-describe descartó race intra-suite. Hipótesis vigentes: (a) el `.or('tutor.is.null,proveedor.is.null')` de PostgREST devuelve la fila F1 aunque ambas estén populadas (posible parsing edge); (b) trigger BD invisible que resetea+re-populate; (c) `necesitaTutor` computado mal por caso borde en el refino JS. Fix en la suite: 2ª corrida cambiada a **dryRun** — verifica el contrato de idempotencia ("elegibles no incluye nuestros ids") sin side effect. Deuda: agregar al endpoint un `?verbose=1` que retorne `elegibles[].id`, `necesitaTutor`, `necesitaProveedor` de cada fila procesada, correr con nuestros 3 ids test, y confirmar si el SELECT de la 2ª corrida los está trayendo. Si sí → bug de idempotencia (fix filter o refino); si no → trigger BD (auditar). Sprint chico post-launch — no bloqueante (el core de idempotencia sí se prueba vía dryRun; el flake real que originó esto solo se dispara al probar con `real` y era un false-positive del assert, no del endpoint).
