...
- **Copy de emails de confirmación de reserva por horas — mostrar rango completo + LAYOUT DE LISTADO** (pedido de PO, 2026-07-28; ampliación 2026-07-28 R4.1 del tren Recordatorios). Hoy los emails de confirmación F1 (reserva por bloque horario, `duracion_min NOT NULL`) muestran la hora de inicio + la duración por separado. El PO pide dos cambios en el mismo retrofit:
  1. **Formato horario**: hora + fin + duración en un bloque legible tipo `"de 14:00 a 15:00 · 1 hora"`. Helpers ya existentes desde R2/R4.1 del tren Recordatorios: `formatBloqueHorario`, `formatBloqueHorarioSinFecha`, `formatHoraCorta`, `formatFechaSinHora` en `lib/formatFecha.ts` (probados en `lib/formatFecha.test.ts` con 60 casos verdes).
  2. **Layout de listado etiqueta/valor**: adoptar el mismo layout que `RecordatorioReservaEmail` (definido en R4.1 del tren Recordatorios, 2026-07-28). Orden fijo del info box: Proveedor (variante tutor) / Cliente (variante proveedor) · Servicio · Fecha · Hora/Horario (u bloque check-in/out F2) · Dónde. La fila "Dónde" usa la cascada canónica: `formatDireccionLinea` (estructurada Ola 1 o legacy) → primera comuna de `servicio.comunas_cobertura` → fallback `"Se coordina por chat con {nombre}"`. Coherencia visual con toda la familia de emails transaccionales (recordatorio + confirmación + cancelación).
  
  Templates afectados: `ReservaConfirmadaTutorEmail`, `AgendamientoTutorEmail` (branch `estado='confirmada'`), `AgendamientoProveedorEmail` (branch `esConfirmadaAuto`), y el equivalente de proveedor si existe. `RecordatorioReservaEmail` (R4.1) ya nace con el layout — sirve de referencia. Testing: render-diff no-regresión F1 con `scripts/render-emails-diff.ts` (script ya extendido con los 7 sets de recordatorio + 16 previos), cuidado especial en branching `esRango` de F2 para no romper la variante ya en prod. Sprint chico post-launch.
