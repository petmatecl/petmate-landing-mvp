...
- **[abierto — PRIORIDAD MEDIA, detectado durante smokes staging admin-visibilidad 2026-08-27]** **Copy "sesión expiró" vs "sin permisos" — separar los dos casos** — pedido PO **2026-08-27**, PARADO durante el sprint `admin-visibilidad` porque el fix NO es trivial ("cambio de string" es engañoso — toca lógica de routing/guards en 3 archivos como mínimo).

  **Motivación PO** (textual): "un no-admin que entra a `/admin` recibe 'Tu sesión expiró. Vuelve a ingresar y te llevamos de nuevo a donde estabas.' Eso es engañoso: manda a revisar la sesión cuando el problema es el rol. Separar los dos casos y para el de permisos usar: 'No tienes acceso a esta sección.' Sin link de reingreso, sin sugerir que reintente. El caso de sesión realmente expirada conserva su copy actual."

  **Hallazgo empírico auditor durante investigación**:
  - **El copy actual "Tu sesión expiró..."** vive en [pages/login.tsx:169](pages/login.tsx#L169) dentro de un banner condicional que se dispara SOLO cuando `router.query.reason === 'expired'`.
  - **Los sitios que setean `?reason=expired`** son puntuales y correctos: (i) 5 sitios de submit de acciones autenticadas cuando detectan session null al vuelo — SolicitarAgendamientoModal x2 (L698, L858), mis-reservas x2 (L216, L311), ServiceFormModal L535, proveedor/index L855; (ii) UserContext SIGNED_OUT handler L331 cuando la sesión se cierra sin logout voluntario Y el user está en un protected path.
  - **Ningún path del rebote admin de no-admin agrega `?reason=expired` hoy**: (i) `pages/admin.tsx` cuando `hasAdminAccess = false` NO redirige a login — muestra login form inline con heading "Acceso restringido" (línea 237); (ii) `components/Shared/RoleGuard.tsx` con `requiredRole='admin'` hace `router.push('/login')` pelado, sin `?reason` (L71). El único page admin que usa RoleGuard es `pages/admin/proveedores.tsx` (el fork de la deuda de dos-listas).
  - **Hipótesis de por qué el PO vio ese copy**: probablemente su sesión de admin genuinamente expiró durante navegación en `/admin` (protected path → UserContext L331 disparó `expired`), no fue el rebote de un no-admin. O el PO estaba logueado como no-admin en `/admin/proveedores` (que sí usa RoleGuard) y su sesión caducó justo cuando el guard evaluó. Sin repro empírico paso a paso, no puedo confirmar el mecanismo exacto — es una atribución que requiere confirmación (corolario P8 11ª).

  **Fix propuesto para separar los dos casos** (NO trivial, toca lógica en 3+ archivos):
  1. **`components/Shared/RoleGuard.tsx`** — cuando `isAuthenticated=true` pero el rol no matchea, redirigir con `?reason=forbidden` (nuevo flag). Cuando `isAuthenticated=false`, mantener `router.replace('/login')` sin flag (o con `?reason=expired` para consistencia con el resto).
  2. **`pages/admin.tsx`** — decisión de diseño: (a) mantener login form inline con copy "No tienes acceso a esta sección" en vez de "Acceso restringido" (cambio dentro del componente, cero routing), O (b) hacer que redirija a `/login?reason=forbidden` igual que RoleGuard y eliminar el form inline (más consistente pero más refactor).
  3. **`pages/login.tsx`** — extender el ternario del banner para leer ambos flags: `reason=expired` → copy actual, `reason=forbidden` → "No tienes acceso a esta sección" (sin link ni sugerencia de reintento como pide el PO).
  4. **Ojo con `UserContext.tsx`** — el handler SIGNED_OUT L331 dispara `?reason=expired` sobre cualquier protected path incluido `/admin`. Ese caso es correcto (sesión genuinamente cerrada), no tocar.

  **Costo estimado**: **~1-1.5 horas** — cambios chicos por archivo pero decisión de diseño (a) vs (b) en admin.tsx afecta consistencia con el resto del panel. Smokes incluyen 3 casos distintos: (i) no-admin logueado navega a `/admin` → ve copy de permisos; (ii) session genuinamente expirada → ve copy de sesión (sin regresión); (iii) admin válido → entra sin rebote.

  **Restricción**: dejado en BACKLOG por instrucción explícita del PO durante el sprint ("Si separar los dos casos NO es un cambio trivial de string y toca lógica de routing/guards, PARÁ y avisame antes de tocar nada — en ese caso va al backlog y aplicamos sin este punto"). Sprint chico dedicado. Prioridad media — el copy actual es misleading pero no bloquea a ningún user real (sale solo en el path que el PO cree, no en el path que efectivamente rebota hoy).
