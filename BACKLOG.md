...
- **Copy de emails de confirmación de reserva por horas — retrofit visual COMPLETO** (pedido de PO, 2026-07-28; ampliación R4.1 layout de listado 2026-07-28; ampliación R4.2 dirección de arte 2026-07-28). Los emails de confirmación (F1/F2/legacy) adoptan el mismo lenguaje visual que `RecordatorioReservaEmail` en 3 capas:
  ...
  Templates afectados: `ReservaConfirmadaTutorEmail`, `AgendamientoTutorEmail` (branch `estado='confirmada'`), `AgendamientoProveedorEmail` (branch `esConfirmadaAuto`), `AgendamientoCancelacionTutorEmail`, y el equivalente proveedor si existe. `RecordatorioReservaEmail` (R4.2) es la referencia canónica. Testing: render-diff no-regresión F1/F2 con `scripts/render-emails-diff.ts` (23 sets: 16 previos + 7 recordatorio), cuidado especial con el pill "MAÑANA" que NO aplica a confirmaciones — reemplazar por un pill contextual ("CONFIRMADA" / "CANCELADA" / etc.) o eliminarlo según decisión PO. Sprint chico post-launch.
