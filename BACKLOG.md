...
- **[abierto — PRIORIDAD ALTA] Mapa de /explorar escapa su stacking context al hacer scroll y se monta sobre el Header** — descubierto por PO **2026-09-04** durante smoke del sprint map-tiles (bug preexistente en prod, no del sprint). Al hacer scroll en `/explorar` con la vista de mapa activa, el mapa Leaflet se despega del contenedor y **tapa la franja superior "Estamos construyendo Pawnecta", los links Blog + Explorar servicios, el botón Ingresar, y los CTAs "Soy tutor" + "Soy proveedor"**. Usuario pierde acceso a navegación y a los 2 CTAs principales de conversión del sitio con un gesto trivial (scroll). En producción, vista pública más visitada. Independiente de qué proveedor de tiles use el mapa (bug reproducible con OSM directo que ya está en /explorar hoy).
  ...
  - **Cobertura verificada de los 3 componentes de mapa del proyecto** (auditor 2026-09-04):
    ...
    - **LocationPicker** [components/Shared/LocationPicker.tsx:143](components/Shared/LocationPicker.tsx#L143): wrapper `relative overflow-hidden` sin z-index ni isolate → **mismo patrón vulnerable que CaregiverMap** → **bug potencial no reportado** (dashboard privado con scroll más contenido, menor exposición).
...
- **[abierto, prioridad media — sprint role-degradation 2026-09-04, bug preexistente] Toast duplicado en `/proveedor` toggle activar/pausar servicio**. Detectado durante smoke de cierre del sprint role-degradation (2026-09-04). Al pausar o activar un servicio del panel proveedor, aparecen **2 toasts distintos por una sola acción**: uno verde con check verde arriba, uno blanco con check negro abajo. Ambas variantes distintas, mismo patrón en "Servicio pausado" y "Servicio activado". Reproducido en producción antes del deploy del sprint role-degradation → **bug preexistente, no introducido por role-degradation**. Hipótesis viva a verificar cuando se abra: el caller llama a `toast()` dos veces (probablemente uno inline optimista + uno post-response). También revisar si el patrón se repite en otros toasts del sitio (grep de callers `toast.success` / `toast.error` en el mismo componente). Sprint chico. Método de descarte de la falsa alarma "regresión del sprint" documentado en `ACTA_ROLE_DEGRADATION.md §10` como caso replicable.
