...
- **[abierto — CONVIENE antes del lanzamiento, PRIORIDAD ALTA, detectado 2026-09-09 durante primera reserva real prod] RES-MASC · Selector de mascota del tutor no filtra por especie/tamaño aceptado del servicio** — Aldo (tutor) reservó el paseo de Eduardo (servicio con `acepta_perros=true` únicamente) y el modal `SolicitarAgendamientoModal` **permitió elegir a Freddie (gato)**. Fix: filtrar el selector de mascotas del tutor por las columnas escalares `acepta_perros / acepta_gatos / acepta_otras` del servicio + los `tamanos_aceptados` (si aplica). Si el tutor no tiene ninguna mascota compatible, mostrar aviso claro y bloquear la confirmación en vez de un select vacío o incoherente. Spec: tutor con gato + perro, servicio solo perros → selector muestra solo el perro. Sprint chico dedicado ~1-2h.
  - **Impacto operativo hoy**: el servicio de Eduardo (paseo) recibió una reserva con mascota gato aceptada por accidente. Cero data corruption (el `mascota_id` guardado apunta a Freddie legítimamente en `mascotas`), pero UX confusa para el proveedor que ve al gato en el detalle de una reserva de paseo canino.

- **[abierto — CONVIENE antes del lanzamiento, PRIORIDAD MEDIA-ALTA, detectado 2026-09-09] MAIL-MASC · Email al proveedor + vista de reserva en panel NO incluyen la mascota** — verificado por PO en primera reserva real prod: el email `AgendamientoProveedorEmail` y la vista de reserva en `/proveedor` tab Solicitudes no muestran nombre/especie/tamaño/notas de la mascota. Para paseo y cuidado es el **dato principal** que el proveedor necesita para prepararse. La data existe (`agendamientos.mascota_id` + `mascotas.*`), sólo falta consumirla en template + render del panel. Fix: bloque "Mascota" en `AgendamientoProveedorEmail.tsx` + sección equivalente en la card de reserva del panel. Sprint chico ~2-3h (template + render + smoke).

- **[CERRADO 2026-09-09 · decisión de producto PO tras primera reserva real prod] Sin email al tutor por su propia cancelación** — cuando un tutor cancela su reserva, NO se le envía email de confirmación de la acción propia. El tutor sabe que canceló (lo hizo activamente desde el modal); el email sería ruido. Los emails de cancelación siguen aplicando para la contraparte (proveedor recibe aviso cuando el tutor cancela; tutor recibe aviso cuando el proveedor cancela). Comportamiento intencional, no falta feature. Anotado por claridad tras la primera reserva real prod donde apareció la duda.

- **[abierto — DESPUÉS del lanzamiento, deuda UX, detectado 2026-09-09 tras primera reserva real prod] MIS-RESERVAS-TABS · Pestañas Próximas/Pendientes/Historial de `/mis-reservas` necesitan más contraste y peso visual** — hallazgo PO tras usar la vista como tutor real. Las 3 pestañas leen apagadas contra el fondo; cuesta identificar en qué tab está el user activamente. Fix propuesto (no diseñar ahora): mayor peso del label activo (bold + color accent + subrayado o pill background), separación clara del inactivo (slate-400 vs slate-800). Sprint chico UX ~30min. Sin urgencia hasta que aparezca un patrón de tutores confundiéndose.

- **[abierto — dato operativo PO, NO tocar código] DATO-EDU · Teléfono de Eduardo en su perfil es placeholder `+56912345679`** — llega tal cual al tutor cuando toca "Contactar al proveedor". Gestión del PO con Eduardo directamente. Anotado sin acción de código.

- **[abierto — DESPUÉS del lanzamiento, deuda UX del editor] EDITOR-UX · Editor de servicio, secciones Agenda / Excepciones / Mascotas con poca separación visual + hints largos** — hallazgo PO 2026-09-09. Los textos de ayuda de franjas y excepciones son párrafos largos que se leen como copy, no como ayuda. Fix propuesto: reducir a una línea de hint + llevar el detalle a tooltip. Adicionalmente, el hint del toggle "Habilitar reservas" ("Confirmas o rechazas cada solicitud desde tu panel") es **falso cuando la agenda real está activa** — debe cambiar según el estado del segundo toggle. **Este último punto (hint dinámico) va al mini-sprint `agenda-categorias`** ya en cola.

- **[abierto — mini-sprint agenda-categorias, encolado después de L1, decisiones producto PO 2026-09-09]** Tres decisiones aprobadas + una adición (EDITOR-UX arriba):
  1. **Ampliar `CATEGORIAS_BLOQUE_HORARIO_F1`** con `fotografia`, `retratos`, `etologia` (servicios de sesión con duración fija). Verificar que ninguna tenga campo específico que choque con la agenda (ej. `horario` texto libre); si lo tiene, apagarlo cuando la agenda real esté activa, como ya hace el hint del bloque C con el bloque A.
  2. **Toggle "Habilitar reservas"**: ocultarlo cuando la categoría NO admite F1 ni F2; en su lugar, línea informativa causa-neutral: "En esta categoría los tutores te envían solicitudes y las respondes desde tu panel."
  3. **Modal legacy "Solicitar servicio"**: reemplazar `<input type="datetime-local">` nativo por el date picker del proyecto (react-day-picker, ya en uso en F2) + selector de hora en pasos de 30 min. Copy y validaciones sin cambios. **Condición de parada**: si implica cambiar el formato que reciben los endpoints o los emails, detener y reportar.
  4. **Adición del hint dinámico** (del ítem EDITOR-UX arriba): el hint del toggle "Habilitar reservas" cambia según estado del segundo toggle — cuando agenda real ON, el hint debe reflejar el flujo instant-book.

- **[abierto — PRIORIDAD ALTA] Mapa de /explorar escapa su stacking context al hacer scroll y se monta sobre el Header** — descubierto por PO **2026-09-04** durante smoke del sprint map-tiles (bug preexistente en prod, no del sprint). Al hacer scroll en `/explorar` con la vista de mapa activa, el mapa Leaflet se despega del contenedor y **tapa la franja superior "Estamos construyendo Pawnecta", los links Blog + Explorar servicios, el botón Ingresar, y los CTAs "Soy tutor" + "Soy proveedor"**. Usuario pierde acceso a navegación y a los 2 CTAs principales de conversión del sitio con un gesto trivial (scroll). En producción, vista pública más visitada. Independiente de qué proveedor de tiles use el mapa (bug reproducible con OSM directo que ya está en /explorar hoy).
  - **Causa técnica probable**: Leaflet usa z-indexes internos altos por default (`.leaflet-tile-pane`=200, `.leaflet-overlay-pane`=400, `.leaflet-popup-pane`=700, `.leaflet-control`=800). Estos son relativos al stacking context del contenedor del mapa. Si el contenedor no establece su propio stacking context, los z-indexes internos "escapan" al context superior y pueden superar el `z-40` del Header sticky.
  - **Cobertura verificada de los 3 componentes de mapa del proyecto** (auditor 2026-09-04):
    - **CaregiverMap** [components/Explore/CaregiverMap.tsx:111](components/Explore/CaregiverMap.tsx#L111): wrapper `relative overflow-hidden` **sin z-index ni isolate** → **NO establece stacking context** → **bug confirmado**.
    - **LocationMap** [components/Shared/LocationMap.tsx:38](components/Shared/LocationMap.tsx#L38): wrapper con `className="isolate"` + `style={ position:'relative', zIndex: 0 }` → **SÍ establece stacking context** → **inmune al bug**. El autor de este componente conocía el patrón.
    - **LocationPicker** [components/Shared/LocationPicker.tsx:143](components/Shared/LocationPicker.tsx#L143): wrapper `relative overflow-hidden` sin z-index ni isolate → **mismo patrón vulnerable que CaregiverMap** → **BUG CONFIRMADO 2026-09-04** durante smoke sprint carto-key. En dashboard sección Ubicación, el mapa se monta sobre el botón "Guardar Cambios" de la barra inferior — **el botón queda tapado por el mapa**. Es PEOR que el caso /explorar (donde tapa navegación): acá tapa el botón que el user necesita para persistir lo que acaba de editar. **El sprint del z-index cubre confirmadamente CaregiverMap + LocationPicker (los 2 sin isolate). LocationMap queda fuera del alcance del fix — ya tiene el patrón correcto.**
  - **⚠️ EL MODELO YA ESTÁ EN EL PROYECTO — LEER ANTES DE ARRANCAR**: el sprint es **copiar la fórmula ya funcional de [LocationMap:38](components/Shared/LocationMap.tsx#L38)**, no inventar solución. Ese componente aplica `className="isolate"` + `style={{ position: 'relative', zIndex: 0, overflow: 'hidden' }}` y es inmune al bug reportado. La solución literal es traer esos dos atributos al wrapper de CaregiverMap (línea 111) y LocationPicker (línea 143). Cero cambio funcional del mapa, cero decisión de diseño, cero riesgo — es transcripción directa de patrón que ya vive en producción sin bug. Smoke visual = scroll en /explorar hasta llegar donde antes tapaba el Header → Header sigue visible.
  - **Smoke crítico al abrir**: scroll en /explorar con mapa activo → header sigue visible. Scroll en dashboard del proveedor con picker de ubicación abierto → header sigue visible. Popups + controles del mapa (zoom, atribución) siguen visibles y clickeables. Móvil + desktop.
  - **Familia del bug**: es el mismo antipatrón del backdrop del NotificationBell resuelto en sprint notifs-panel C7a — stacking context mal resuelto que se resolvió con portal al `document.body`. Acá la resolución probable es distinta (isolate al wrapper, no portal) porque el mapa es contenido, no overlay.
  - **Sprint chico dedicado** (~30 min), post-sprint map-tiles. Cero interacción con qué proveedor de tiles se use (independiente del URL).

- **[abierto — PRIORIDAD ALTA] Mapa en modo degradado por falta de API key** — pedido de PO **2026-09-04** (durante smoke del sprint role-degradation). Observado en producción, `/proveedor/ed384fc0-826f-44e9-bedf-f03f4eb68ca3` sección Ubicación: el mapa renderiza con la marca de agua **"API KEY REQUIRED"** repetida por todo el fondo + link `carto.com/basemaps/apikey`. El proveedor de tiles rechaza la petición por falta de autenticación.
  - **Consecuencias**: (a) cualquier persona que mire un perfil de proveedor ve un cartel técnico en inglés atravesando el mapa — impresión de sitio roto; (b) si el proveedor de tiles corta el servicio por límite de uso, el mapa puede dejar de cargar del todo.
  - **A investigar cuando se abra**: qué proveedor de mapas usa el proyecto (probable react-leaflet con tiles CARTO por el link), si la clave nunca se configuró o si venció, plan gratuito con límites (CARTO tiene tier gratuito con quota), dónde va la variable de entorno (probable `NEXT_PUBLIC_*` para uso client-side).
  - **Contexto**: PO lo notó mirando el perfil de un proveedor nuevo que se registró en prod — **hay actividad orgánica**, conviene que el perfil se vea bien.
  - **DECISIÓN 2026-09-04 (revisada dos veces — anotable en el acta del futuro sprint)**: Opción A (conseguir API key CARTO), NO Opción B (migrar a OSM directo). Primera lectura del PO privilegió B (sin claves, unifica estilo con /explorar, cero dependencia). Auditor arrancó ejecución en rama `map-tiles` (SHA `caf3972` en remoto, NO mergeada). PO hizo comparación justa **misma zona (Las Condes)** en prod (CARTO) vs preview (OSM) y revirtió: CARTO se ve mejor para el bloque Ubicación (menos ruido, colores suaves, círculo verde del área aproximada resalta; OSM lo hace competir con calles naranjas + nombres de vías). La primera comparación era inválida — miró perfiles de zonas distintas. Costo asumido conscientemente: CARTO retira raster en algún momento → sprint futuro obligatorio (ver entrada nueva más abajo). Prefiere el sitio se vea bien HOY con proveedores reales publicados y pagar migración cuando toque. **Ejecución cuando llegue la clave del PO**: branch nueva desde main (NO reusar `map-tiles`), `NEXT_PUBLIC_CARTO_KEY` env var, agregar `?key=${...}` a URLs de tiles en LocationMap + LocationPicker, key en Vercel Prod + Preview + Development, smoke completo (mismo perfil sin marca de agua + attribution visible + picker con F5 persistencia).

- **[abierto — PRIORIDAD MEDIA, trigger cuando CARTO anuncie retirement de raster tiles] Migrar de tiles raster CARTO a alternativa estable** — anotado 2026-09-04 tras decisión revisada del PO de mantenerse en CARTO con API key (Opción A del sprint map-tiles). CARTO está retirando activamente sus raster basemaps públicos (`basemaps.cartocdn.com/rastertiles/*`), sustituyéndolos por vector basemaps. Timeline oficial no confirmado, pero avisos internos de CARTO en 2024/2025 indican que raster tiles quedarán deprecated. Cuando aterrice el aviso oficial o el servicio empiece a degradar, hay que migrar.
  - **Trabajo YA HECHO como plan B**: la rama `map-tiles` (SHA `caf3972` en remoto — **NO borrar**) tiene la migración a OpenStreetMap directo lista para mergear. 3 líneas de código + 1 texto de attribution en 2 archivos (LocationMap + LocationPicker). Verificado build P1.1 exit 0. Si CARTO corta antes de tiempo, mergear esa rama es cuestión de minutos + smoke visual.
  - **Otras alternativas si OSM no alcanza cuando llegue el momento**: Stadia Maps (free 200k tiles/mes con API key, estilos Alidade/Stamen similares a Voyager), Mapbox (free 50k map loads/mes con API key, estilos muy pulidos). Ambas requieren decisión de estilo visual + registro + env var + costo si crecemos mucho.
  - **Trigger**: (a) email/blog post oficial de CARTO anunciando deprecation date, (b) primer degradation observable (tiles rotos, latencia alta, tile subset faltante), (c) decisión proactiva de reducir dependencia externa. Cualquiera venga primero.
  - **Costo estimado**: sprint chico (~30 min) si se elige OSM (rama ya lista); ~1-2 horas si se elige otro proveedor (registro + env var + comparación visual).

- **[abierto — PRIORIDAD MEDIA-ALTA — camino de salida en página de conversión] Botón "Volver" del perfil de proveedor lleva fuera del sitio** — descubierto por PO 2026-09-04 en `/proveedor/ed384fc0-826f-44e9-bedf-f03f4eb68ca3`. Click en "Volver" → aterriza en página de inicio de Google (u origen externo del referrer). **Lectura del PO sin verificar aún** (a validar cuando se abra): el botón usa `router.back()` / `window.history.back()` en vez de una ruta fija. Si el user llegó al perfil desde una búsqueda externa (Google, redes) o escribiendo la URL directamente, "atrás" lo saca del sitio.
  - **Impacto**: alto para tráfico orgánico. Alguien que llega desde Google a un perfil, tocando "Volver" esperando ver más proveedores del sitio → **se va**. Es camino de salida en página de conversión (perfil de proveedor es donde el usuario evalúa si contactar).
  - **Fix propuesto para cuando se abra**: (a) opción simple — "Volver" siempre navega a `/explorar`; (b) opción con memoria — usar `router.back()` solo si `document.referrer` es del mismo origen (`window.location.origin`), sino fallback a `/explorar`. La (b) preserva UX del usuario que navegó dentro del sitio + protege al que llegó de afuera. Ambas son cambios chicos.
...
- **[abierto — prioridad MEDIA (subida de baja 2026-09-04 tras zoom PO en imagen), contraste UI + inconsistencia con sistema de diseño] Botón "Ver perfil completo" del popup del mapa `/explorar` con texto poco legible** — detectado por PO 2026-09-04 durante smoke z-index-maps + confirmado al ampliar la imagen del popup. En el popup que aparece al hacer click en un marker de precio, el botón "Ver perfil completo" tiene texto en **verde apenas más claro que el fondo verde del botón** — cuesta leerlo. Es **inconsistencia con el sistema de diseño**, no decisión: comparar con botones verdes del resto del sitio ("Soy tutor" / "Soy proveedor" del header, "Aceptar todas" del banner de cookies, botones de acción de las tarjetas de servicio) que usan **texto blanco pleno** y se leen sin esfuerzo. Es el **CTA principal del popup** — el botón que lleva del mapa al perfil.
...
- **[abierto — prioridad baja, encuadre imagen] Foto del servicio en el popup del mapa `/explorar` queda mal encuadrada** — detectado por PO 2026-09-04 durante smoke z-index-maps al ampliar la imagen del popup del marker de $50k. La foto del servicio en el popup se ve **recortada arriba y abajo**, y el sujeto (hombre con perro en el caso observado) queda **descentrado hacia la derecha**. Probable causa: `object-position` por defecto (`object-center` default centra la imagen, no el sujeto) o relación de aspecto del contenedor que no calza con la de las fotos subidas.
  ...
  - **Fix por contenedor**: cambiar `h-28` a `h-32`/`h-36` para más altura + agregar `object-top` o `object-center` según lo que se vea mejor. Cero decisión de diseño mayor.
  - **Fix estructural (relacionado con entrada existente L529)**: **cropper interactivo estilo LinkedIn/Instagram al subir foto** — el proveedor ajusta el encuadre al aspect ratio del contenedor antes del upload. Si esa entrada se aterriza, este problema **se reduce solo** — los proveedores pueden centrar el sujeto explícitamente. Anotar cross-reference en ambas direcciones.
...
- **[abierto — PRIORIDAD MEDIA] Mapa en la ficha de servicio según modalidad** — pedido de PO **2026-09-04** (durante sprint carto-key, mirando el sitio en general). Hoy el mapa vive solo en `/proveedor/[id]`. Las fichas de servicio (`/servicio/[id]`) no lo tienen. **La respuesta correcta no es "sí" ni "no" — es "depende de quién se traslada"**.
  - **Casos analizados por PO**:
    ...
    - **CASO 2 — servicio en el lugar del proveedor** (ej. hospedaje hogareño, servicio `c1000001`, modalidad "En la casa del cuidador"): el tutor lleva su mascota hasta allá. Muestra solo comuna "Providencia" que es grande — no es lo mismo cerca de Tobalaba que cerca de Salvador. Alguien que va a dejar su perro necesita saber si le queda cerca. **Mapa SÍ aporta y hoy falta**.
  - **Propuesta**: mostrar mapa en `/servicio/[id]` **solo cuando la modalidad implique que el tutor se traslada al lugar del proveedor**. El dato de modalidad ya existe y se muestra en la ficha → se puede condicionar sin agregar campos.
  - **A definir al abrir**:
    - Qué modalidades exactas califican. "En la casa del cuidador" seguro. Revisar catálogo completo de modalidades del proyecto (buscar en `lib/camposPorCategoria.ts` o similar) y proponer lista.
    - Si se reusa `components/Shared/LocationMap.tsx` tal cual, o necesita variante (los datos vienen del proveedor asociado al servicio, no del servicio mismo).
    - Privacidad: el `Circle` de área aproximada 1000m que ya usa LocationMap resuelve esto — cero necesidad de nuevo diseño.
    - Complementariedad con la entrada existente **"Ubicación legible del proveedor en su perfil"** (mostrar comuna + región como texto): las dos van juntas. Un tutor evaluando servicio necesita **mapa + texto de comuna**, no uno o el otro.
  - **Origen — corrección explícita del PO**: primera respuesta del PO fue "está bien como está" (mirando solo Caso 1). Al mirar Caso 2 revisó y llegó a "depende de la modalidad". **Registro explícito del cambio de opinión anotado por pedido del PO** — la respuesta simple "sí/no" habría sido incorrecta en ambas direcciones. Es evidencia del valor de pasar por varios casos antes de decidir un feature transversal.

- **[abierto — PRIORIDAD MEDIA] Ubicación legible del proveedor en su perfil** — pedido de PO **2026-09-04** (mismo smoke). Hoy el bloque Ubicación muestra un círculo de área aproximada + copy "Área aproximada en Santiago. La dirección exacta no se comparte públicamente." El problema: "Santiago" es demasiado amplio — para saber dónde está realmente el proveedor hay que interpretar los nombres de calles del mapa, que pide esfuerzo y conocimiento de la ciudad.
  - **Fix propuesto**: mostrar **región + comuna** visibles como texto en el perfil. No reemplaza al mapa, lo complementa.
  - **Dato disponible**: `proveedores.comuna` y `proveedores.region` existen en el schema (verificado durante sprint admin-visibilidad — el RPC las devolvía). El dato existe y no se está mostrando en la ficha pública.
  - **Decisión de producto pendiente al abrir**: (a) dónde exactamente va — junto al nombre / dentro del bloque Ubicación / en ambos, (b) si mostrar comuna compromete la promesa de privacidad del copy actual. Lectura preliminar: comuna ya se muestra en las tarjetas de búsqueda (`ServiceCard`), entonces exponerla en el perfil individual no agrega superficie de exposición — a confirmar al abrir el sprint.
  - **Contexto**: mismo perfil del proveedor orgánico nuevo que disparó ENTRADA 1 arriba. Los 2 pedidos son sobre la misma ficha.
...
- **[abierto — PRIORIDAD ALTA, sprint de investigación en curso, rama `cuelgue-diag`]** **Cuelgue intermitente de carga (spinner indefinido, se destraba con Ctrl+Shift+R)** — hallazgo del PO durante smokes prod del sprint admin-visibilidad. **NO introducido por ese sprint** — reproducido en múltiples rutas no relacionadas y en ambos entornos. Prioridad ALTA (por encima de session-timeout-fix).
  ...
  **HECHOS ACUMULADOS (verificados por el PO en el navegador, 2026-08-28)**:
  ...
  2. Ocurre en **PROD y STAGING**.
  3. **Spinner INFINITO**. Sin toast de error, sin excepción en consola, sin request fallido reportado. Única cosa en Console: warning de `feature_collector.js` sobre parámetros deprecados — no parece relacionado pero **anotado como observación pendiente**.
  4. Se destraba con **Ctrl+Shift+R**. Recarga normal **no siempre** destraba.
  5. Cuando la página SÍ carga, el RPC responde sano: `admin_listar_proveedores` status **200 en 81 ms, 2.7 kB**. En el mismo trace: **"Finish: 5.91 s"** con timeline extendido hasta **80.000 ms**.
  6. **Upstash respondió** durante el cuelgue: el chip "Rate limit: Upstash" YA muestra su valor (14-30 ms). Header renderizó; **lo que no resuelve es el contenido**.
  ...
  **DATOS FALTANTES ordenados por costo-beneficio** (para el sprint de investigación):
  ...
  - Estado de **document.readyState** + eventos `DOMContentLoaded`/`load` en el momento del cuelgue.
  - Estado de los **contexts React** (isLoading del UserContext, isOpen del FeedbackContext) en el momento del cuelgue — el spinner del componente mira `loading` local, pero podría estar bloqueado por un context upstream que nunca resolvió.
  - Estado de las **promesas pending** en el heap (DevTools > Memory heap snapshot durante el cuelgue).
  - Repro controlado con timestamps + secuencia de acciones exacta previa al cuelgue.

  **Contexto del proyecto** (referencia, sin conclusión): PWA activa en prod via `@ducanh2912/next-pwa@10.2.9`. Ver CLAUDE.md > "PWA / Service Worker". El SW existe y funciona, pero fue descartado como causa.

  **Rama de investigación**: `cuelgue-diag` (12 chars, DNS-safe) desde `main @ 979e693` — creada para instrumentación diagnóstica futura. El PO aprueba cualquier instrumentación antes de mergear.
...
- **[CERRADO — L1-3 sprint launch-l1, 2026-09-11 merge SHA `71853fd` (PR #17)] TIM-1 · Timeout de inactividad por check-al-mount** — fix aterrizado del `session-timeout-fix` (commit histórico `5c7c2d1`) portado a `components/SessionTimeout.tsx` (commit `a577b88`): reorden de listeners dentro de `init()` DESPUÉS del check async, cleanup solo si listenersRegistered. Cero race con mousemove pre-check. Regresión spec `e2e/specs/launch-l1/tim1-session-timeout-reorder.spec.ts` con 2 tests: (a) marker 20 min atrás + F5 → expulsa a `/security-logout` + marker removido; (b) marker fresco + F5 → sigue en `/proveedor` (no expulsa). **Trayecto del PR**: 3 iteraciones antes del verde. (i) `a577b88` fix del componente + spec inicial; test 1 timeouteó 2 veces por `page.mouse.move` pre-reload que disparaba los listeners de MOUNT 1 y pisaba el marker antes del F5 (falso negativo por diseño del test — mecanismo de verificación instrumentalmente inválido, P8 aplicado a e2e). (ii) `1ab7ffe` quité mouse.move — test 1 pasó 2s, pero destapó cascada de ~20 tests colaterales fallando con "Auth session missing!" porque `handleLogout()` ejecutó `supabase.auth.signOut()` scope global e invalidó el JWT del `e2e/.auth/proveedor.json` que toda la suite comparte via setup project. (iii) `9677ed1` `page.route('/auth/v1/logout', → 204)` al inicio del test → SDK resuelve local, redirect ocurre, server nunca recibe la request, JWT vive → 4 checks verdes. **Lección incorporada al skill/CLAUDE.md**: test debe validar el fix real (no un proxy) + side-effects destructivos ocultos por otro bug reaparecen al arreglar el bug de arriba. Historia original preservada:
- **[HISTÓRICO] Timeout de inactividad por check-al-mount estaba roto desde su primer commit** — el path "F5 tras 10+ min sin actividad → expulsión a `/security-logout`" NUNCA funcionó en la práctica. Detectado por Aldo con smoke positivo-conocido (seteo marker a hace 20 min, F5 en `/proveedor`, verificar expulsión); resultado: no expulsó, marker se pisó a NOW post-recarga. Bug preexistente, no del sprint email-landing.
...
- **[CERRADO 2026-09-09 — primera reserva real en prod verificada empíricamente]** **AGE-1 · Agenda F1/F2 activa en prod desde julio 2026**. Estado previo "3/25 cobertura, Eduardo sin agenda F1/F2 activada" era descriptivo obsoleto — la agenda YA estaba activada desde la migration F2 del sprint julio (`f2-prod-20260728`). Verificado por PO 2026-09-09 con **primera reserva real end-to-end**: Aldo tutor → servicio de paseo `35c16aee-a471-4517-a0b8-56383dc6b579` (Eduardo Cano) → viernes 11-sep 14:00 → **confirmada al instante** + emails a tutor y proveedor recibidos. Cero código nuevo, cero SQL. El estado "abierto" en la entrada previa era 15ª instancia del patrón "estado obsoleto en BACKLOG". Contexto operativo del snapshot 2026-08-25 preservado por trazabilidad histórica:
  ```
...
- **[CERRADO — L1-4 sprint launch-l1 2026-09-11, verificado noop — ya mergeado a prod en `panel-prov-fixes-prod-20260827` SHA `83d9312`]** `ServiceFormModal` pide MASCOTAS DOS VECES en categoría Etología, genera datos inconsistentes. **Cierre**: el sprint L1 listó ETO-1 como pendiente, verificación 2026-09-11 muestra que la Opción 1 (recomendada por auditor) ya fue aplicada por el sprint `panel-prov-fixes` (commit `83d9312`, tag `panel-prov-fixes-prod-20260827`) — el preset `etologia` de `lib/camposPorCategoria.ts` ya no incluye `especies_atendidas` (ver comentario documentado en [lib/camposPorCategoria.ts:368-380](lib/camposPorCategoria.ts#L368-L380)). Icon map huérfano `especies_atendidas: Stethoscope` (L722) sigue como deuda cosmética anotada más abajo, sin bloqueo funcional. Historia original preservada: Detección de Aldo: el formulario tiene el campo base "Mascotas aceptadas" (Perros/Gatos/Otras, marcado obligatorio) + el campo específico de la categoría Etología "Especies que atiendes" (mismas 3 opciones). Usuario puede marcar Gatos en uno y Perros en otro; el sistema guarda dos respuestas contradictorias en columnas distintas. **Más grave que la validación de descripción y la race del select de Categoría**.
...
- **[abierto — PRIORIDAD ALTA, revisar ANTES del lanzamiento, detectado durante apply prod del sprint admin-visibilidad 2026-08-27, ampliado 2026-09-01 durante F2B sprint notificaciones]** **Default privileges divergentes entre staging y prod: en AMBOS entornos, toda función nueva del schema public nace con EXECUTE otorgado a `anon` Y `authenticated`** — hallazgo empírico del PO durante apply de la migration `20260827_admin_listar_proveedores_rpc.sql` en prod. **Ampliación 2026-09-01** durante apply de F2B (`20260828_notify_proveedor_new_eval_oficializar.sql`): el patrón NO se limita a `anon` — también aplica a `authenticated`. En admin-visibilidad se detectó solo el caso `anon` porque era el que fallaba el smoke; F2B destapó que `REVOKE FROM PUBLIC + FROM anon` deja `authenticated_puede=true` hasta agregar `REVOKE FROM authenticated` explícito. **Verificación empírica staging 2026-09-01**: post `CREATE FUNCTION notify_proveedor_new_eval` + `REVOKE FROM PUBLIC + FROM anon`, `has_function_privilege('authenticated', ..., 'EXECUTE')` retornó `true`; tras `REVOKE FROM authenticated`, retornó `false`. Los 3 REVOKE deben ir juntos (PUBLIC + anon + authenticated) en toda migration nueva mientras no se resuelva la deuda estructural. Adicionalmente, la migration F2B en `main @ 47ec797` fue actualizada con los 3 REVOKE (commit siguiente) como plantilla.
...
- **[abierto — PRIORIDAD MEDIA (subida desde BAJA 2026-08-28 tras segunda superficie confirmada), detectado durante smokes staging admin-visibilidad 2026-08-27 + preview cuelgue-diag 2026-08-28]** **Scroll no vuelve al tope al cambiar de tab — patrón general de navegación por tabs** — hallazgo inicial en `/admin`, ampliado a segunda superficie que revela un problema del PATRÓN, no un caso puntual.

  **Superficie 1 — `/admin`** (2026-08-27, sprint admin-visibilidad): al alternar entre tabs del panel admin (Métricas / Proveedores / Feedback / etc.), el scroll queda en la posición del tab anterior en vez de subir al tope del contenido del tab nuevo. Fricción moderada cuando el tab anterior tenía scroll largo (ej. lista completa de proveedores) y el nuevo tab tiene contenido corto.

  **Superficie 2 — Panel de proveedor `/proveedor`** (2026-08-28, preview cuelgue-diag, sesión admin): con tabs Servicios / Perfil / Evaluaciones / Mensajes / Métricas / Solicitudes. Click en tab "Evaluaciones" mantiene la posición del scroll anterior — PO aterrizó mirando el FOOTER, tuvo que subir a mano para ver el contenido del tab que acababa de abrir. **Agravante**: el área de contenido de Evaluaciones se ve VACÍA en la captura → al aterrizar abajo NO hay señal visual de que haya algo más arriba, la página se lee como si fuera solo footer. **A verificar en el sprint**: si Evaluaciones estaba realmente vacía o si el contenido estaba arriba fuera de viewport (segundo agravante distinto: empty state incorrecto vs bug de scroll).

  **Qué cambia el patrón (subida de prioridad BAJA → MEDIA)**: dos superficies distintas afectadas con arquitecturas de tabs distintas (`/admin` con sidebar sticky vertical + `activeTab` state, `/proveedor` con tabs horizontales + su propio state manager) apuntan a **problema del patrón de navegación por tabs**, no caso aislado. Con dos rotas confirmadas, es probable que estén todas.

  **Pauta al abrir el sprint**:
  1. Relevar TODAS las superficies del sitio con navegación por tabs — grep por `activeTab`, `useState.*[Tt]ab`, componentes tipo Tabs/TabPanel — no solo las conocidas.
  2. Verificar el comportamiento de scroll en cada una.
  3. Diseñar fix como **patrón común** (componente wrapper, hook custom `useScrollToTopOnTabChange(activeTab)`, o utility en el layout compartido), NO como parche por pantalla. Si el patrón se implementa una vez, futuros tabs quedan cubiertos automáticamente.

  **Fix candidato (no diseñar ahora)**: hook `useEffect(() => window.scrollTo({ top: 0, behavior: 'smooth' }), [activeTab])` extraído a `lib/hooks/useScrollToTopOnTabChange.ts`. Cero migración BD. Costo grueso ~30 min (hook + retrofit en 2 superficies conocidas + auditoría de resto) + smoke visual por superficie.

  **Prioridad MEDIA** — no bloquea funcionalidad, pero afecta UX en múltiples pantallas de forma consistente. Sigue por debajo del cuelgue intermitente. Sprint chico dedicado o piggyback en próximo touch a alguno de los paneles con tabs.
...
- **[abierto — hallazgo colateral error-audit] Archivos huérfanos en bucket `avatars`** — dos superficies confirmadas:
  - **[a1]** `components/Client/ClientLayout.tsx:handlePhotoUpload` — **CERRADO 2026-09-08 SHA `f40f499`** por reorden de operaciones (verificar rol antes del upload). Bug era 100% reproducible en cada intento de tutor de subir foto (no limitado a error path).
  - **[a2]** `pages/proveedor/index.tsx:778-809:uploadAvatar` — abierto. Mismo antipatrón "upload antes de UPDATE" pero mitigado por `{ upsert: true }` + path determinista `${user.id}/avatar.${fileExt}`. Impacto bajo: máximo ~5 huérfanos por proveedor (una por extensión que probó y luego cambió). Fix futuro con misma técnica del reorden.
  - **Pregunta que hay que responder cuando se abra el sprint de limpieza**: **¿cuántos archivos huérfanos hay hoy en el bucket `avatars` en producción?** Si el bug (a1) estuvo activo desde que existe la función y hay proveedores reales usando el sitio, puede haber acumulación no trivial. Cross-check propuesto: listado del bucket + `SELECT auth_user_id, foto_perfil FROM proveedores UNION SELECT auth_user_id, foto_perfil FROM usuarios_buscadores` (si esta última tuviera la columna a futuro) → todo archivo no referenciado = huérfano. Saber el número cambia si es limpieza de rutina o un problema mayor. La respuesta también informa la política operativa: ¿se borran o se dejan? ¿se borran solo los > N días sin referencia?

- **[abierto — hallazgo colateral error-audit] Bucket `avatars` — policy SELECT posiblemente demasiado amplia** — Supabase Studio advierte que la política SELECT actual permite a cualquier cliente listar todos los archivos del bucket. Riesgo: enumeration del bucket por terceros (nombres de archivo, patrones `${user.id}-*`). Revisar la policy y acotarla sin romper `getPublicUrl()` que necesita SELECT anónimo por convención pública del bucket. Fix probable: mantener SELECT anónimo solo sobre `object` (get file individual por ruta conocida) y bloquear LIST del bucket a rol `anon`. Requiere auditar el fix contra el flujo actual de renderizado (cada `<Image src={publicUrl}>` en el sitio es un GET del archivo por ruta, no un LIST — debería seguir funcionando).

- **[abierto — hallazgo colateral error-audit — decisión de producto pendiente] Avatar del tutor en `ClientLayout` es trigger que siempre falla** — el `<label htmlFor="avatar-upload" className="md:hidden ...">` en [components/Client/ClientLayout.tsx:158](components/Client/ClientLayout.tsx#L158) muestra un avatar clickeable **solo en mobile viewport**. En desktop no renderea. Al hacer click, el flow siempre termina en el modal "No disponible aún — La foto de perfil todavía no está habilitada para tutores." porque `usuarios_buscadores` no tiene columna `foto_perfil` (solo `proveedores` la tiene). **Decisión de producto pendiente**:
  - **Opción A** — habilitar foto de perfil para tutores: agregar columna `foto_perfil` a `usuarios_buscadores`, extender UX de subida al desktop también, feature paridad con proveedor.
  - **Opción B** — quitar el trigger: eliminar el `<label>` + `<input>` del `ClientLayout`, dejar solo el avatar de solo lectura (sin click). El feature no aporta valor si nunca funciona.
  - Sin resolver este ítem, cada tutor mobile que descubre el trigger toca "cambiar foto" y aterriza en un modal de "no disponible" — expectativa fallida sin próxima acción.

- **[abierto — hallazgo colateral error-audit] Código muerto por retiro de `/usuario` (commit `4d0f42d`, abril 2026)** — el redirect server-side `/usuario → /explorar` en [next.config.js:206-210](next.config.js#L206-L210) hace que `pages/usuario.tsx` sea **inalcanzable en producción vía la ruta declarada**. Cascada de código muerto:
  - **`pages/usuario.tsx`** — la ruta redirige antes de montar. El archivo solo se ejecuta si alguien remueve el redirect (regresión). Candidato a eliminación.
  - **`components/Client/DashboardContent.tsx`** — importado solo por `pages/usuario.tsx` (grep confirma cero otros callers). Si `pages/usuario.tsx` se elimina, este archivo queda huérfano — candidato a eliminación en la misma limpieza.
...
- **[abierto — hallazgo colateral error-audit — auditoría completa de destructurings ignorando `.error`] Lista línea a línea de los 62 callers Tipo B/C/D restantes** — el sprint error-audit cerró los 6 Tipo A + 5 tests excluidos. Resto (62 callers en ~28 archivos) queda documentado acá para que el próximo sprint arranque sin re-auditar. Clasificación provisional del auditor — el próximo sprint puede reclasificar en la apertura si algún caller tiene impacto distinto al que le atribuí.
  ...
  **Tipo B (~39 líneas, moderate — degradación user-facing de features)**:
  > **Estado sprint tipo-b — CERRADO 2026-09-09**: Fase 0 base compartida + inventario cerrados en PR #7. Lote 1 (dashboard proveedor) PR #8 SHA `ae3dbef`. Lote 2 (tutor panel — favoritos + hallazgo dashboard dead-code) PR #9 SHA `adece0b`. Lote 3 (fichas — PreguntasSection + ReviewList + ServiceDetailView 6 callers + fix guard mount de ReviewList) PR #10 SHA `5220cec`. Lote 4 (admin notificaciones — stats compacto + banner actividad) PR #11 SHA `0f2fcd7`. Lote 5 (helpers + context + badges — silent + log Sentry) PR #12 SHA `ebd16e4`. Tag prod: `tipo-b-prod-20260909`. Acta: [ACTA_SPRINT_TIPO_B.md](ACTA_SPRINT_TIPO_B.md). 29 callers Tipo B originales + 3 adyacentes detectados → 32 líneas tocadas total.
    ...
    - `lib/authService.ts:50, 72` — auth utilities. **[cerrado — Lote 5 SHA `ebd16e4`, silent + log Sentry (fetchProfile: fallback null por diseño, orphan guard toma acción)]**
    - `lib/profileUtils.ts:19, 28` — profile utilities compartidas. **[cerrado — Lote 5 SHA `ebd16e4`, silent + log Sentry (fallback textual "Proveedor"/"Usuario" ya presente en callers)]**
    - `lib/hooks/useFavoritos.ts:63` — hook de favoritos. **[cerrado — Lote 5 SHA `ebd16e4`, silent + log Sentry por decisión PO (corazón inline sin espacio para banner)]**
    ...
    - `contexts/UserContext.tsx:749` — query DB del context (no confundir con :739 que es `auth.getSession`). **[cerrado — Lote 5 SHA `ebd16e4`, silent + log Sentry (refreshProveedorRow preserva `proveedorRow` anterior ante fallo)]**
    ...
    - `pages/admin/notificaciones.tsx:41, 51` — listado admin de notifs. **[cerrado — Lote 4 SHA `0f2fcd7`, feed unificado con banner "No pudimos cargar la actividad reciente" + 2 counts adyacentes (:22, :29) también migrados con EstadoErrorCompacto]**
    ...
    - `components/Shared/UnreadBadge.tsx:16` — badge (silent 0 count ante error). **[cerrado — Lote 5 SHA `ebd16e4`, silent + log Sentry para `conversations` (:16) + `messages` count adyacente (:29); preserva count previo ante fallo]**
    - `components/Service/PreguntasSection.tsx:36` — preguntas del servicio. **[cerrado — Lote 3 SHA `5220cec`, banner "No pudimos cargar las preguntas" reemplaza lista Q&A]**
    - `components/Service/ReviewList.tsx:64` — lista de reviews (join proveedores). **[cerrado — Lote 3 SHA `5220cec`: fotos hydration silent + log Sentry (avatar-inicial fallback); query principal evaluaciones (:46) también migrada a runReadQuery + banner "No pudimos cargar las evaluaciones", además guard mount `totalReviews > 0` removido en `ServiceDetailView:1435` para exponer error path]**
    ...
    - `components/Servicio/ServiceDetailView.tsx:186, 292, 330, 340, 421, 432` — ficha de servicio (varios paths de contacto, favoritos, evaluaciones, chat). **[cerrado — Lote 3 SHA `5220cec`: deep-link scroll (:186) + vinculo buscador/agendamiento (:330, :340) silent + log Sentry; chat lookup (:292) + count rate limit (:307, detectado durante review) + review gate (:421, :432) toast fail-close con copy "No pudimos abrir tu chat / verificar tu contacto" — antes fallo silente creaba conv duplicadas o mensajero engañoso "solo puedes evaluar a proveedores contactados"]**
...
- **[CERRADO — sprint L1-2 CASE-6, 2026-09-09 SHA `<tbd>`] Caso 6 — `pages/api/auth/signup.ts:220`** — cerrado con: (a) destructurado `.error` del lookup + Sentry `captureMessage('signup_provider_lookup_failed')` con tags subsystem/table/route/errorCode; (b) nuevo tag distinto `signup_provider_row_missing` para el edge case `{data:null, error:null}`; (c) endpoint `notify-nueva-solicitud` extendido con modo DEGRADADO — acepta `{ fallback: { email, nombre, apellido_p, rut?, comuna? } }` sin providerId, emite subject `"[DEGRADADO] Nueva solicitud..."` + Sentry `admin_notify_degraded`. Con esto, aunque el lookup falle, el admin recibe la notificación con los datos del propio signup (email + nombre), marcada como parcial. Contract spec `e2e/specs/launch-l1/case6-notify-degraded.spec.ts` verifica los 3 modos (normal, degradado, body inválido) + auth. Impacto: cero solicitudes silenciosas al admin aunque falle la query del providerId. **Historia original preservada** para trazabilidad:
  ...
  - **Fix propuesto para cuando se aborde**: `Sentry.captureMessage('signup_notification_query_failed', { tags: { providerId: userId }, extra: {...} })` en el path de fallo. Verificación end-to-end via dashboard Sentry en producción con evento real (no smoke sintético).
  - **Trigger de reapertura**: si aparece patrón de solicitudes de proveedores sin email al admin → priorizar el Sentry breadcrumb como diagnóstico. Trigger observable, no calendario.

- **[CERRADO — sprint f2-ci-fix 2026-09-09, señal visible aplicada]** **Servicios existentes con `descripcion` < 100 caracteres — guard con señal visible siempre** — política PO 2026-09-09: se mantiene el mínimo de 100 chars (no se permite guardar otros campos sin extenderla), pero el guard debe dar señal visible en TODO caso, no solo toast (que dura ~4s y se pierde si el proveedor no mira arriba). Contexto: FIX 3 de panel-prov-fixes (`ab82c86`, 2026-08-27) agregó el guard `if (descripcion.trim().length < 100) return toast.error(...)` en `handleSubmit` de `ServiceFormModal.tsx:625` — solo toast. Durante el diagnóstico del sprint f2-ci-fix, la sesión MCP tras Guardar con desc 55 chars + wait 18s no mostró señal alguna: toast expirado, ni inline error, ni focus, ni scroll. Un proveedor que toca Guardar sin recibir señal es una pantalla muda. **Fix aplicado en `ServiceFormModal.tsx`**: extender el guard al patrón F2-2B-B ya probado — nuevo state `descripcionError` + inline error persistente bajo el textarea (border danger + aria-invalid + role=alert) + scrollIntoView + focus del textarea + toast (los 4 canales). Limpieza del error inline al primer typeo. Cerrado por `<SHA-lote-5-tipo-b-tbd>` (ver commit `fix(editor)` del PR ci-f2-specs). Test de regresión: `e2e/specs/f2-2b/s7-guard-descripcion-corta.spec.ts` (2 tests: guard cierra fail-close con inline+focus+cero-PATCH; typeo limpia inline). Impacto en prod: cero regresión — el único caso conocido (servicio "Cuidado tu mascota" del propio Aldo, inactivo) sigue funcionando igual, pero ahora ve el error inline si intenta editar sin extender descripción.
...
- **[abierto, prioridad baja — evita repetir el bug del contraste del popup mapa] Extraer componente `<Button>` canónico compartido** — anotado 2026-09-04 en ronda 1 del sprint popup-fix. Descubierto al diagnosticar el bug del contraste del botón "Ver perfil completo" del popup del mapa: el proyecto **NO tiene componente `<Button>` compartido** — cada botón se define a mano con Tailwind. Los 3 botones canónicos comparados (Header "Soy proveedor", CookieBanner "Aceptar todas", popup mapa) usan **EXACTAMENTE las mismas clases** (`bg-accent-600 text-white ... hover:bg-accent-700 transition-colors`). La diferencia entre bien-legible y mal-legible NO estaba en el diseño del botón — estaba en el contexto CSS del contenedor (Leaflet `.leaflet-popup-content a` sobrescribía `text-white` por especificidad, ver `ACTA_POPUP_FIX.md`). **Con un `<Button variant="primary">` compartido**, el mismo patrón defensivo (`!text-white` o CSS namespace-safe) vive una sola vez y protege contra el próximo contenedor con CSS agresivo. Sin él, cada botón nuevo puede sufrir el mismo bug silente en otro contexto. **Alcance**: extraer un componente `components/UI/Button.tsx` con variants primary/secondary/ghost/danger; refactorear los ~20+ botones del sitio para usarlo; deprecar (con comentario) los patrones ad-hoc. Sprint chico (~2-4 horas). Trigger para arrancar: (a) aparición de otro bug del mismo tipo en otro contenedor CSS agresivo, (b) sprint de sistema de diseño, o (c) higiene proactiva post-viaje PO.

- **[abierto, deuda light — 1 línea] Import muerto de `Popup` en `components/Shared/LocationMap.tsx`** — detectado 2026-09-04 durante grep del sprint popup-fix. `LocationMap.tsx:1` importa `Popup` de `react-leaflet` pero **cero usos en el archivo** (grep `<Popup` = 0 matches). Import quedó residual de alguna refactor previa. Sacar de paso cuando se toque el archivo por otra razón — no amerita sprint dedicado. Fix: 1 char (remover `Popup, ` del import de react-leaflet).
...
- **[abierto — DECISIÓN CONSCIENTE, NO PENDIENTE OLVIDADO — prioridad baja] Rotación de `service_role` key tras hallazgo `notify_viaje_publicado`**. Contexto en [ACTA_NOTIFS_PANEL.md §5.1](ACTA_NOTIFS_PANEL.md). Función `SECURITY DEFINER` con HTTP POST a dominio externo (`n8n.vectis-workspace.cl`) + token hardcodeado, descubierta y drop'eada en el sprint notifs-panel. Cierre 2026-09-01: origen identificado (ex socio del PO, proyecto propio de "viajes" sobre la infraestructura compartida). Accesos revocados: colaborador GitHub eliminado, NO figura en Supabase Team. Barrido de rastros en prod+staging: cero funciones/tablas/http_requests relacionados, la función NUNCA ejecutó. **Decisión de NO rotar `service_role` por ahora** con razonamiento explícito: (a) rotarla exige regenerar el JWT secret → invalida anon key, service_role y todas las sesiones activas → obliga a actualizar env vars en Vercel de inmediato o el sitio se cae; (b) el ex socio no tiene acceso a Supabase; (c) la función nunca ejecutó (cero evidencia de que tuvo la clave). **Trigger para reevaluar**: cualquier indicio de acceso no autorizado, request anómala en logs Supabase/Vercel, o alerta de Sentry con signature sospechosa → aplicar procedimiento canónico (rotar JWT secret desde Supabase Dashboard → actualizar `SUPABASE_ANON_KEY` + `SUPABASE_SERVICE_ROLE_KEY` en Vercel envs Production+Preview+Development sincronizado → redeploy inmediato → verificar login funciona → invalidar sesiones residuales). NO pendiente olvidado — decisión evaluada.

- **[abierto, prioridad media — sprint notifs-panel + default-privs, 2026-09-01] Revisar si extensión `pg_net` debe seguir habilitada**. Era la extensión que permitía el POST saliente de `notify_viaje_publicado` (función dropeada, ver [ACTA_NOTIFS_PANEL.md §5.1](ACTA_NOTIFS_PANEL.md)). Hoy nada la usa (verificado por PO en prod + staging 2026-09-01: `net._http_response`=0 filas, `net.http_request_queue`=0 filas, cero funciones referencian `net.http_*`). Mantener activa una capacidad que nadie necesita es superficie gratis para el próximo abuso. **Antes de desactivar**: confirmar que Supabase no la use internamente para features propias (webhooks del dashboard, notificaciones de sistema, integraciones oficiales). Método: consultar docs Supabase sobre uso interno de `pg_net`, y si no aparece dependencia explícita, `DROP EXTENSION pg_net CASCADE` en staging primero para verificar que nada se rompe, luego prod. Sprint chico post-viaje del PO.

- **[abierto — sprint dedicado post-viaje PO, sprint role-degradation 2026-09-04] Auditoría de callers `.data` sin `.error` en el proyecto**. De 329 llamadas `await supabase`, ~26 sitios en 12 archivos destructuran `{ data }` **sin** manejar `.error`. `UserContext.hydrateFromSession` era el más grave (impacto sistémico — cualquier admin con corte de red de 2s durante login veía pantalla afirmando falsamente "no completaste tu perfil"), aterrizado en sprint role-degradation C2b. El resto son operaciones específicas con blast radius acotado a su componente — cada uno amerita decisión propia: retry, `.error` handling defensivo, o aceptar el comportamiento actual. Ver `ACTA_ROLE_DEGRADATION.md §5` para tabla completa. **Prioridades sugeridas al abrir**:
  - **P2 — [lib/apiAuth.ts:65](lib/apiAuth.ts#L65) `isAdmin` server-side**. Marcado aparte por encima de los otros 25 (P3) porque su modo de falla afecta a todos los endpoints admin del proyecto. **Fail-closed**: con error de query devuelve `false` → admin real tratado como no-admin, endpoint responde 401/403 → UX del admin, no seguridad. Cero riesgo de escalación (nadie gana permisos que no tenía). Fix de 3 líneas — chequeo explícito de `.error` antes del `if (!data) return false;`. Ver `ACTA_ROLE_DEGRADATION.md §6` para análisis completo.
  - **P3 — Resto (25 sitios en 11 archivos)**. `components/Servicio/ServiceDetailView.tsx` (6), `components/Client/DashboardContent.tsx` (4), `components/Client/ClientLayout.tsx` (3), `components/Admin/ConversionMetrics.tsx` (2), `components/Shared/RoleGuard.tsx` (2), `contexts/UserContext.tsx` refreshProfile + refreshProveedorRow (2 — no el hydrate, ya cubierto), `lib/authService.ts` (2 — dead code, ver ítem aparte), `components/Proveedor/CertificacionesSection.tsx` (1), `components/Service/PreguntasSection.tsx` (1), `components/Service/ReviewList.tsx` (1), `components/Shared/UnreadBadge.tsx` (1).

- **[abierto, sprint chico — role-degradation 2026-09-04] `lib/authService.ts` completo es dead code**. Duplica `UserCapabilities` + `GUEST_CAPABILITIES` + `deriveCapabilities()` sin ningún consumidor externo. Verificado durante C1 del sprint role-degradation por 3 vías de grep (exhaustividad + destructuring de `useUser()` en 20 sitios). Removible entero. Sprint chico post-viaje PO.

- **[abierto, prioridad media — sprint role-degradation 2026-09-04] `components/SessionTimeout.tsx:104` — mismo patrón silente**. Catch NO expulsivo tras un `getSession()` fallido en el guard de inactividad, diseñado a propósito para no expulsar. **Pero también silente**: si `getSession()` tira, el guard de inactividad queda **inservible para esa sesión hasta reload** y nadie se entera. Es el hermano del bug que arreglamos en UserContext — otra superficie con degradación silenciosa. Fuera del alcance de role-degradation por decisión del PO ("mezclarlos ensucia el smoke"). Sprint chico post-viaje.

- **[CERRADO — verificado 2026-09-09 sprint launch-l1 · fix estructural ya vivía en prod desde toast-fix `450ba46` (2026-09-04)] Toast duplicado en TODAS las acciones del sitio**. Grep exhaustivo del código actual (`grep -rn "<Toaster" --include='*.tsx' --include='*.ts'`) devuelve **1 solo match real** en [pages/_app.tsx:80](pages/_app.tsx#L80), sin `richColors`. El match residual de `pages/completar-registro.tsx:344` es dentro de un comentario del sprint toast-fix. El reporte del PO post-fix (actualización sprint z-index-maps 2026-09-04) es una 14ª instancia del patrón "estado obsoleto en BACKLOG" que la cláusula de fidelidad del PDPO reconoce — el fix ya estaba en prod cuando el PO reportó los "3 casos". Red de seguridad para reintroducción: spec [e2e/specs/launch-l1/toast-dup-regresion.spec.ts](e2e/specs/launch-l1/toast-dup-regresion.spec.ts) — 2 tests: (a) `expect([data-sonner-toaster]).toHaveCount(1)`, (b) acción desde ServiceFormModal (guard descripción-100) dispara EXACTAMENTE 1 `li[data-sonner-toast]` con el texto esperado. Cerrado en L1-1 SHA `<tbd>`. **Casos históricos preservados para trazabilidad**:
  - 3 casos confirmados en 2 pantallas + 2 entornos (última actualización sprint z-index-maps 2026-09-04):
  1. **Toggle activar/pausar servicio** en `/proveedor` (panel proveedor) — **producción**. Detectado en smoke role-degradation 2026-09-04.
  2. **Guardar perfil** en el dashboard — **preview**. Detectado en smoke carto-key 2026-09-04.
  3. **Guardar perfil** en el dashboard — **producción**. Detectado en smoke z-index-maps 2026-09-04.
  Mismas 2 variantes visuales en los 3 casos: **uno verde con check verde arriba, uno blanco con check negro abajo**.
  - **Descarta que sea del entorno preview** (el caso 3 lo confirma en prod).
  - **Refuerza la hipótesis de infraestructura, no de caller**. Si fuera un caller llamando `toast()` dos veces, tendría que ser el **mismo bug repetido en 3 lugares independientes** — muy improbable. Una causa única en infraestructura (2 `<Toaster>` montados en el árbol) explica los 3 casos con una sola fuente.
  - **Hipótesis viva sostenida**: probablemente **dos `<Toaster>` montados** — uno de sonner "canónico" en `_app.tsx:63` + otro inadvertidamente en un componente hijo. El de `ServiceFormModal.tsx:2578` (conocido como "duplicado a limpiar" en la entrada de homologación toasts del PDPO) es candidato principal, pero puede haber más en otros componentes. Cada Toaster renderiza cada toast disparado → dos elementos visuales por invocación.
  - **Método verificable al abrir**: `grep '<Toaster' **/*.tsx` = espera 1 solo match en `_app.tsx`. Cada match adicional es Toaster duplicado a remover.
  - **Sprint chico (~15-30 min)**: `grep` + remover Toaster duplicado + smoke confirmando 1 toast por acción en las 2 pantallas conocidas + probar 2-3 acciones adicionales (evaluación, contacto, etc.) para descartar más candidates de duplicación.
  - **Prioridad ALTA** — afecta a toda la app, no una pantalla. 3 casos en 2 días la sostiene.
  - Método de descarte de falsa alarma "regresión del sprint" documentado en `ACTA_ROLE_DEGRADATION.md §10` como caso replicable.

- **[abierto, prioridad media — sprint default-privs 2026-09-01] Auditoría de los corolarios P8 de CLAUDE.md**. Cada corolario numerado (P8 5ª, 6ª, 8ª, 10ª, 11ª — todos escritos en CLAUDE.md L519-624) está anclado a un único incidente. Revisar uno por uno: ¿se observó más de una vez? ¿se verificó o se infirió? ¿sigue siendo cierto? Los que no resistan, retractar con nota (patrón opción Y del sprint default-privs — preservar texto original + nota de retractación). Los que sí, anotar la evidencia que los sostiene. Incluye evaluar si la numeración formal conviene, o si da apariencia de rigor que el respaldo no tiene. Origen: sprint default-privs 2026-09-01, a partir del caso del corolario P8 6ª (SQL Editor rollback silente entre corridas) retractado — la afirmación se sostenía sobre 1 incidente aislado, el PO la refutó por observación empírica en admin-visibilidad, la retracción verbal no aterrizó al documento, y reapareció como fundamento de diseño en la Ronda 2 de default-privs.

- **[abierto, sprint propio — pospuesto por PO en default-privs 2026-09-01] Alcance C — auditoría RPC-callable case-by-case en schema public**. 204 funciones RPC-callable en `public`, 203 con EXECUTE anon. Mayoría genuinamente necesita el grant (búsqueda pública sin loguear, acciones del usuario logueado). Alguna proporción NO — identificarlas requiere cruzar los `supabase.rpc(...)` del frontend con la lista completa de RPCs, y aplicar REVOKE explícito a las que sobran. Costo estimado: horas de auditoría case-by-case. Modo de fallar feo: revocar una función que SÍ se llama desde cliente rompe producción en silencio (el cliente recibe 401/403 al llamarla, sin error visible hasta que el user la usa). Método sugerido: (a) `grep -rE "supabase\.rpc\(['\"]([^'\"]+)" pages/ components/ lib/ | sort -u` para lista de RPCs usadas; (b) contrastar contra `SELECT proname FROM pg_proc WHERE pronamespace = 'public'::regnamespace ORDER BY proname;`; (c) para cada RPC no usada desde cliente, verificar si se usa server-side (grep de `supabaseAdmin.rpc`) o desde otro RPC; (d) para las que ni cliente ni server usan, `REVOKE EXECUTE FROM anon, authenticated;`. Sprint dedicado post-viaje del PO. Origen: sprint default-privs 2026-09-01 (alcance A+B aterrizado, C pospuesto por costo + risk profile + viaje del PO).

- **[abierto, sub-item cierre default-privs] Reportar a Supabase docs: guía hardening-data-api incompleta**. Los 4 statements de https://supabase.com/docs/guides/database/hardening-data-api usan `IN SCHEMA public` (verificado con WebFetch 2 veces con foco específico durante sprint default-privs). El 4to statement (`revoke execute on functions from public`) es exactamente el que el manual PostgreSQL cita como no-op (per-schema no puede remover privileges del setting global). Consecuencia: los usuarios que sigan la guía al pie de la letra creen que cerraron la exposición de funciones y no la cerraron. Adicionalmente los statements enumeran privilegios en vez de usar `REVOKE ALL`, dejando TRUNCATE/REFERENCES/TRIGGER/MAINTAIN residuales en tablas y UPDATE residual en secuencias. Reportar como GitHub issue en supabase/supabase con evidencia + cita literal manual PostgreSQL. Cuando el PO abra el issue.

- **[P3, cosmético UX notif — sprint notifs-panel 2026-09-01]** Reservas F2 (rango de noches) muestran `00:00` en el panel de notificaciones porque el helper `formatFechaRelativa(modo:'evento')` renderiza `HH:MM` desde el timestamp del evento, y para F2 no hay hora — el timestamp es midnight local del día de check-in. Se ve como "Mañana a las 00:00" en vez de algo tipo "Mañana (llegada)". Fix propuesto: pasar un flag `sinHora?: boolean` al helper (o segundo helper `formatFechaEventoSinHora`) que omita el `, HH:MM` para F2. Detectado en ronda 1 del sprint notifs-panel al validar defecto 7. No bloqueante — el 00:00 es correcto técnicamente (el evento inicia a las 00:00), solo lee raro. Sprint chico.

- **[P3, cosmético UX notif — sprint notifs-panel 2026-09-01]** Contraste visual insuficiente entre "no-leída apagada" (dot grande + fondo tint) y "leída apagada" (sin dot, fondo blanco). En el panel corto de notificaciones (que ahora muestra solo no-leídas por decisión D2), cuando el user está al día (0 no-leídas), el empty state es correcto — pero al pasar hacia el panel completo `/notificaciones` (deuda pendiente sprint dedicado) o durante la transición de una notif que se marca leída, la diferencia entre estados apagados es demasiado sutil. Fix propuesto: agregar hairline izquierda `border-l-2 accent-500` a las no-leídas (además del dot), o cambiar el fondo tint de las no-leídas a un tint más saturado. Sprint chico cuando se toque el diseño del panel largo.
