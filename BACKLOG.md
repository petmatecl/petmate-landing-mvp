...
- **[CERRADO — L1-3 sprint launch-l1, 2026-09-11 SHA `<tbd>`] TIM-1 · Timeout de inactividad por check-al-mount** — fix aterrizado del `session-timeout-fix` (commit histórico `5c7c2d1`) portado a `components/SessionTimeout.tsx`: reorden de listeners dentro de `init()` DESPUÉS del check async, cleanup solo si listenersRegistered. Cero race con mousemove pre-check. Regresión spec `e2e/specs/launch-l1/tim1-session-timeout-reorder.spec.ts` con 2 tests: (a) marker 20 min atrás + mousemove previo + F5 → expulsa a `/security-logout` + marker removido; (b) marker fresco + F5 → sigue en `/proveedor` (no expulsa). Historia original preservada:
