...
- **`components/Proveedor/ServicioDetallesForm.tsx` huérfano**: tras la consolidación del tab "Info del Servicio" (commit de este sprint), el componente quedó sin consumidores. Su JSDoc anticipaba uso en "Step 3 del wizard de registro (cuando se integre)" pero `pages/register.tsx` L546-550 documenta explícitamente que Sprint 4 Fase 1 movió esa captura a `ServiceFormModal`. Es decir, el uso descrito por el JSDoc nunca se implementó. Preservado en el codebase por instrucción del último sprint pero sin consumidores reales — candidato a borrar en el próximo pase de limpieza si no se reactiva el path del wizard.
