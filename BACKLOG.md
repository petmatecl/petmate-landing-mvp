...
- **[abierto — PRE-LANZAMIENTO alto ROI] Notificaciones a admin de solicitudes pendientes** — pedido de PO **2026-08-11**. Hallazgo en prod: Aldo encontró **8 solicitudes de alta de proveedores pendientes** en /admin, la más antigua del **28-jun-2026** (~6 semanas sin respuesta). Con lanzamiento a semanas: dejar proveedores esperando es costoso — son la gente que necesitamos a bordo.
...
- **[abierto, SEVERIDAD ALTA — sistémico BD] Foreign keys ausentes en TODAS las tablas del proyecto** — hallazgo detectado 2026-08-14 durante Ola 1 C1 fix (400 pestaña Conversión). Verificación MCP staging: consulta `information_schema.table_constraints WHERE constraint_type='FOREIGN KEY' AND table_schema='public'` sobre las 12 tablas relevantes (`agendamientos, contactos, conversations, evaluaciones, eventos_tracking, favoritos, mascotas, messages, notificaciones, proveedores, servicios_publicados, usuarios_buscadores`) retorna **CERO fks definidas en ninguna**. Consecuencias:
  1. **Ningún embed PostgREST con sintaxis `!fk_name(...)`** funciona en toda la app — cualquier query admin/panel que use ese patrón devuelve 400. C1 solo fue el primer síntoma detectado.
  2. **Filas huérfanas posibles en todas las tablas**: al borrar un proveedor, sus agendamientos/servicios/contactos/evaluaciones quedan con `proveedor_id` apuntando a nada. La app hace lookup con `.maybeSingle()` y toma `null` como "no encontrado", pero la data integrity depende 100% de que la app no borre mal — no de la BD.
  3. **ON DELETE CASCADE no existe** — cleanup manual + errores silentes por diseño.
  4. **A2 inventario data prueba (Ola 1)** — borrar los proveedores ejemplo requiere DELETE en cascada manual sobre 4-5 tablas (agendamientos, servicios_publicados, contactos, evaluaciones, favoritos). Sin FK ON DELETE CASCADE, es error-prone.
  - **Fix**: sprint dedicado post-launch. Migration `ALTER TABLE ... ADD CONSTRAINT ... FOREIGN KEY (...) REFERENCES ...` para cada relación identificada. Ver `lib/types/agendamiento.ts` + schema de cada tabla para mapear las relaciones lógicas. Estimado: 4-6h investigación + migration + testing (staging + prod). NO trivial — hay que verificar que no haya rows huérfanas actuales que la migration rechace.
  - **Workaround interino** (patrón Ola 1 C1): queries admin usan lookup cliente-side sin embed. Deuda cognitive por cada nuevo panel.
  - **Trigger**: si aparece otro caso de 400 en pestaña admin o si la integridad se rompe visible (proveedor huérfano en agendamiento activo). Sino, post-launch.
