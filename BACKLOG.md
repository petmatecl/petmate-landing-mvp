...
- **[abierto — prioridad baja, contraste UI] Botón "Ver perfil completo" del popup del mapa `/explorar` con texto poco legible** — detectado por PO 2026-09-04 durante smoke z-index-maps. En el popup que aparece al hacer click en un marker de precio, el botón "Ver perfil completo" tiene texto en un tono claro sobre fondo verde y **cuesta leerlo**. Comparar con los botones verdes del resto del sitio, que usan texto blanco pleno — ejemplo canónico: CTAs "Soy tutor" / "Soy proveedor" del header + botones de acción de las tarjetas de servicio. **Fix probable**: revisar la clase del texto del botón en el HTML del popup (probablemente en el JSX del `<Popup>` en `components/Explore/CaregiverMap.tsx` ~línea 169+) — reemplazar `text-white/N` u opacity reducida por `text-white` pleno. Cero cambio funcional, solo tokens de color. Sprint chico de contraste UI (podría entrar junto con otros ítems de UI/paleta pendientes en BACKLOG).
