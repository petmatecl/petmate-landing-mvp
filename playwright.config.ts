...
export default defineConfig({
    ...
    projects: [
        ...
        },
        {
            ...
            testIgnore: /specs[\\/](f2-3|f2-recordatorios-cron|producto-2|conviene|visual|visits-doble|chat-open-error|incidente-usuario-fix)[\\/]|specs[\\/]error-audit[\\/]c5-.*\.spec\.ts$|specs[\\/]tipo-b[\\/].*tutor.*\.spec\.ts$/,
        },
        {
            name: 'chromium-tutor',
            use: {
            ...
            // Sprint conviene Paso 0 (2026-09-11) — specs de diagnóstico
            // bajo /specs/conviene/ corren como tutor puro (Camila). Ver
            // e2e/specs/conviene/estado-actual.spec.ts para detalle del
            // criterio de rol.
            testMatch: /specs[\\/](f2-3|producto-2|conviene|chat-open-error|incidente-usuario-fix)[\\/].*\.spec\.ts$|specs[\\/]tipo-b[\\/].*tutor.*\.spec\.ts$/,
        },
        {
