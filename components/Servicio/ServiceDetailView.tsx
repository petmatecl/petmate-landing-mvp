...
import ProveedorResumenCard from './ProveedorResumenCard';
...
export interface ServiceDetailViewProps {
    ...
    isExample?: boolean;
    // Rediseno Commit 4: rating global del proveedor para la tarjeta resumen
    // Zona B. Fetched en getServerSideProps de /servicio/[id] con query
...
export default function ServiceDetailView({
    ...
    // Bottom sheet mobile — se abre desde la barra fija inferior cuando el
    // tutor tap "Agendar"/"Contactar". Concentra todas las acciones de
    // contacto (Agendar/Mensaje/WhatsApp/Telefono) en un solo punto de
    // entrada mobile. Desktop no lo usa (tiene su sticky right).
    ...
    return (
        ...
        <div className="bg-slate-50 pb-8 lg:pb-0">
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="w-full lg:w-2/3 flex flex-col gap-8">
                        ...
                        <section
                            ...
                            className="relative focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-600 focus-visible:rounded-2xl"
                        >
                            {/* Foto principal */}
                            <div
                                ...
                                <div className="flex gap-2 mt-3 overflow-x-auto pb-1 px-4 sm:px-6 lg:px-0">
                                    {service.fotos.map((foto: string, i: number) => (
                                        <button
                                            ...
                                            className={`shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${fotoActiva === i ? 'border-accent-600 opacity-100' : 'border-transparent opacity-60 hover:opacity-90'
                                                }`}
                                        >
                                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            ...
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm">
                                ...
                                {totalReviews > 0 && (
                                    <span className="inline-flex items-center gap-1 text-slate-600">
                                        <Star size={14} className="text-accent-600 fill-accent-600" />
                        ...
                        */}
                        ...
                        {(() => {
                            ...
                            return (
                                <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
                                    <h3 className="text-xl font-semibold text-slate-900 mb-4 flex items-center gap-2">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-accent-600"><polyline points="20 6 9 17 4 12" /></svg>
                                        Qué incluye
                                    </h3>
                                    {chips.length > 0 && (
                                        ...
                                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {items.map((inc, i) => (
                                                <li key={i} className="flex items-start gap-2 text-slate-600">
                                                    <svg className="w-5 h-5 text-accent-600 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>
                                                    <span>{inc}</span>
                                                </li>
                                            ))}
                                        </ul>
                        ...
                        {service.disponibilidad && (() => {
                            ...
                            if (parsed && typeof parsed === 'object') {
                                ...
                                return (
                                    <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
                                        <h3 className="text-xl font-semibold text-slate-900 mb-4 flex items-center gap-2">
                                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-accent-600"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                                            Disponibilidad
                                        </h3>
                        ...
                        {/* 8. Zona B — Tarjeta resumen del proveedor.
                            ...
                            "Sobre X" que tenia bio expandible + ExpandibleText.
                            Aca solo: foto + nombre + badges verificacion + rating
                            global + anios experiencia + primeros auxilios + bio
                            corta (150 chars sin expand) + boton "Ver perfil
                            completo →". La bio larga + certificaciones tabla +
                            presencia online + mapa + idiomas + todos sus servicios
                            viven en /proveedor/[id]. */}
                    ...
                    <div className="hidden lg:block w-full lg:w-1/3 space-y-6">
                        ...
                        >
                            ...
                            {/* PRECIO + VisitCounter — action panel del sticky
                                right, ahora SOLO desktop (la columna outer es
                                hidden lg:block). En mobile el precio vive en la
                                barra fija inferior y las visitas en la linea
                                sutil post-hero. */}
                            <div>
                            ...
                            {/* ── PROVEEDOR — bloque eliminado en Commit 4 del
                                rediseno estructural. Contenido migrado a
                                <ProveedorResumenCard> en la columna izquierda
                                (Zona B). Aca en la sticky right queda solo el
                                action panel (precio + CTAs). El detalle completo
                                del proveedor (bio larga, certificaciones tabla,
                                presencia online, mapa, idiomas, todos sus
                                servicios) vive en /proveedor/[id]. */}
            ...
            {/* Sticky CTA bar — solo mobile. Ahora es un TRIGGER del bottom
                sheet: el boton abre el sheet con todas las opciones (Agendar
                primario si habilitado + Mensaje + WhatsApp + Telefono). El
                precio queda a la izquierda como social proof del momento de
                accion. z-40 sobre headers sticky (z-30) pero bajo modales
                (z-50). El sheet abre en z-50 encima de todo. */}
            ...
            {/* Bottom sheet mobile — concentra todas las acciones de contacto
                (Agendar/Mensaje/WhatsApp/Telefono) en un unico punto de entrada
                mobile. Se abre desde la barra fija inferior. Respeta los gates
                de guest y isExample (los handlers hacen su propio branching:
                guest → LoginRequiredModal; isExample → ExampleCTAModal). El
                sheet se cierra antes de disparar la accion — la modal correspon-
                diente aparece encima limpiamente. Excepcion: PhoneRevealButton
                revela el numero adentro del sheet (no lo cerramos para que el
                tutor pueda ver/copiar el numero antes de cerrar manualmente). */}
            <MobileActionSheet
