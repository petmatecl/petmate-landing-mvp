    ...
    return (
        <div className={`flex flex-col items-center text-center p-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 relative ${isDark ? 'bg-white/10' : 'bg-emerald-100'}`}>
                <div className={`absolute -top-3 -right-3 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm border-4 ${isDark ? 'bg-emerald-700 border-slate-900 text-white' : 'bg-emerald-700 border-slate-50 text-white'}`}>
                    {paso}
                </div>
