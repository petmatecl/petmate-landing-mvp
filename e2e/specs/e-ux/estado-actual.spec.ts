...
// Refactor 2026-09-15 (sprint E DUP-CAMPOS-CATEGORIA): el DO block del archivo tiene el orden `RAISE EXCEPTION 'DUP-CAMPOS-CATEGORIA: ...'` (RAISE antes que el tag, no después); assertion actualizada al orden real del literal.
test('[e-ux DUP-CAMPOS-CATEGORIA] migration file existe con assertion post-migración', async () => {
    ...
    // DO $$ assertion post-migración: RAISE EXCEPTION con tag DUP-CAMPOS-CATEGORIA
    // (el RAISE viene antes que el tag dentro de la string del mensaje).
    expect(source, 'DO $$ block con RAISE EXCEPTION tag DUP-CAMPOS-CATEGORIA').toMatch(
        /DO\s*\$\$[\s\S]*RAISE EXCEPTION[\s\S]*DUP-CAMPOS-CATEGORIA/,
    );
});
