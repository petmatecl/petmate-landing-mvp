-- Create table for storing sitter availability
...
-- Policy: Sitters can manage their own availability
CREATE POLICY "Sitters can manage own availability" 
ON public.sitter_availability 
FOR ALL 
USING (auth.uid() = sitter_id);

-- Policy: Public/Clients can view availability (for potential future direct calendar view)
CREATE POLICY "Public can view availability" 
ON public.sitter_availability 
FOR SELECT 
USING (true);

-- Create RPC function significantly optimized for date range intersection
-- This function returns sitter profiles that match the filters AND are available for ALL days in the range
