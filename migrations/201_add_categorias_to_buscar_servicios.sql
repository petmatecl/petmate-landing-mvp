-- Migration 201: Agregar parámetro p_categorias al RPC buscar_servicios
-- Ejecutar en Supabase SQL Editor (o via CLI: supabase db push)
-- Reemplaza el parche de parallel-queries en explorar.tsx con filtrado nativo.

-- IMPORTANTE: Esta migration debe ejecutarse después de verificar la firma
-- actual del RPC en el schema dump. Ajustar el bloque CREATE OR REPLACE
-- para incluir TODOS los parámetros existentes más el nuevo p_categorias.

-- Ejemplo de firma actualizada (adaptar según firma real del RPC):
CREATE OR REPLACE FUNCTION buscar_servicios(
    p_categoria_slug text DEFAULT NULL,
    p_categorias     text[] DEFAULT NULL,   -- NUEVO: array de slugs
...
RETURNS TABLE (
    -- Mismas columnas que la función original
    servicio_id          uuid,
    titulo               text,
    descripcion          text,
