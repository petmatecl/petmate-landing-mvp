...
**Rerun autónomo acotado a infra inequívoca — ENMIENDA A P11 (2026-09-11)**: para PRs que **NO tocan código productivo relacionado** con el fail, y cuya evidencia del fail es **inequívocamente de infraestructura de terceros** (Cloudflare 5xx, SSL handshake failure, runner caído, DNS_PROBE_FINISHED_NXDOMAIN, Supabase 525/502/504 wall-to-wall), el auditor puede **relanzar UNA vez** (`gh run rerun --failed`) sin pedir GO explícito, siempre que reporte la evidencia + el `run_id` en el turno. Si vuelve a fallar con el mismo patrón, **escala al PO** y no relanza más — puede ser problema estructural (staging saturado, ventana de incident del proveedor) que amerita fix del ambiente, no polling. Si el PR **sí toca código productivo** relacionado con lo que falla, la regla P11 completa vuelve a aplicar: cero rerun sin GO. Incidente que originó la enmienda: PR #18 (L1 wrap-up, 2026-09-11) — docs-only + 2 SQL files no aplicados por CI. Playwright fail 5m58s con `supabase.co | 525: SSL handshake failed` wall-to-wall en el log. Cero código productivo → GO explícito del PO relajó P11 para este subset de fails. Regla anterior queda intacta como default para casos ambiguos o PRs con código.

**Fallo en spec pre-existente que el PR no toca — AMPLIACIÓN P11 (2026-09-11)**: cuando el CI del PR devuelve rojo con **un solo test fallando** y ese test pertenece a un spec **pre-existente que el PR no toca**, la enmienda de rerun autónomo NO habilita rerun ni merge automático. En su lugar habilita un **diagnóstico de 10 minutos con artifacts**: descargar `test-results/*/error-context.md` + `test-failed-*.png` + `trace.zip`, verificar (a) BD del recurso que el spec necesita para descartar fixture silent-fail, (b) hipótesis específica del PO si dio una (ej. reloj/timezone/borde de ventana), (c) posibles regresiones del propio PR sobre el flow del spec fallado. **Si el diagnóstico muestra flake de fixture (setup no dejó BD/state esperado)**, se corrige el fixture en el mismo PR. **Si muestra bug productivo latente que el PR no toca**, se anota como deuda separada + escala al PO para decisión (rerun manual, merge con override P11 documentado, o pausa para investigación aparte). **Si en 10 minutos no se logra determinar**, se escala al PO con la evidencia recopilada — cero rerun autónomo, cero merge con rojo sin GO. Incidente que originó la ampliación: PR #20 (pan-1-def3 hotfix, 2026-09-11) — post fix def3 con 3 tests propios verdes, 1 fail restante en `e2e/specs/f2-3/s7-cancelacion-fuera-ventana.spec.ts` (spec pre-existente, PR no toca cancelación F2). Diagnóstico via artifacts + BD confirmó fixture OK (reserva insertada con estado=confirmada) y descartó hipótesis reloj/borde; el fail fue `card not visible` con panel /mis-reservas en "Cargando tus reservas..." indefinido — stall del fetch cliente, no determinable en 10 min sin dedicarle sprint aparte. Escalado al PO para decisión.

**Fixmes uno por uno con diagnóstico propio en el acta — ENMIENDA P11 (adición 2026-09-24)**: cuando un run del CI devuelve **múltiples fails** (>1 test) y varios son pre-existentes que el PR no toca, la ampliación P11 anterior aplica **a cada test individualmente**, nunca en lote. Reglas operativas:
- **Fixme es uno por uno, con diagnóstico propio en el acta**. Cada `test.fixme` con etiqueta `[b3-discovered-<YYYY-MM-DD>]` viene con su comentario multi-línea (hipótesis + link a artifact + estado del intento) + su fila propia en la tabla `docs/sprints/bloque-j-4.md > B3 discovered fixmes`. Cero fila colapsada "los siguientes 4 tests…".
- **Nunca en lote por "el PR no toca eso"**. Que un PR no toque la superficie no es evidencia de que el fail sea flake — puede ser regresión indirecta (env var nueva, migration lateral, fixture cross-file). Cada test necesita su propia justificación causal antes del fixme.
- **Antes de cualquier fixme, verificar si el spec pasa en los últimos runs verdes** (`gh run list --workflow <name> --status success --limit 5 --json databaseId,headBranch,headSha,createdAt`). Si el test **pasaba** en runs verdes recientes de PRs no relacionados → **no es deuda histórica: es regresión o ambiente, y el diagnóstico individual decide cuál**. La distinción importa porque cambia la acción: regresión del PR actual (o de un merge lateral) requiere fix del código productivo o del test, no fixme; ambiente (concurrencia cross-PR, saturación Supabase, race timing) sí es candidato a evidencia en `bloque-j-4.md > CI-CONCURRENCY` y decisión de tolerar/aplicar concurrency estructural. La evidencia va al acta como run_ids concretos + branches donde pasó. Si el test **también falla** en algún run verde reciente (por definición no fue "verde" — hay que revisar `--status failure`), la hipótesis de flake se debilita → escalar diagnóstico más profundo antes del fixme.
- **Si tras la verificación los N tests pasan en runs verdes recientes**: NO se aplica fixme en lote — se anota el episodio en el acta `bloque-j-4.md > CI-CONCURRENCY` con los run_ids como evidencia de flakiness cross-PR, se escala al PO con la lista + evidencia, y se espera la decisión sobre si aplicar CI-CONCURRENCY estructural antes de merge o si tolerar el flake con override P11 explícito (justificado en el PR).

Incidente que originó la enmienda: PR #90 (chat-open-error, 2026-09-24) — run `36040364041` con 8 fails, 1 propio + 7 pre-existentes (4× `pan-1/def3-bell-user-context`, 1× `launch-l1/toast-dup-regresion`, 2× `tipo-b/lote-4-admin-notificaciones`). Verificación cross-run: los 7 pasaron en los 5 runs verdes previos del workflow (`sentry-boundary @ a99fe8c/bc367ad/d41ce1a`, `cue-1-fix @ a469b5a`, `button-unif-doc @ 7d39d4a`). El auditor había propuesto fixme en lote con "el PR no toca esas superficies" como justificación — el PO rechazó: cada test necesita diagnóstico propio o evidencia empírica de flake cross-run antes del fixme.

**Nunca `| tee` sin `set -o pipefail`, y bumps de dependencia que cambien requisitos de runtime se verifican contra el Node de Vercel antes de mergear — REGLA PERMANENTE (P12, 2026-09-17)**: dos disciplinas ligadas por el incidente que las originó.

Primero: **cero `| tee` (o cualquier pipe) en un step de GitHub Actions sin `set -o pipefail` al inicio del `run:`**. Sin `pipefail`, el exit code del pipe es el del último comando (habitualmente `tee`, siempre 0), y un test que falla queda enmascarado — el step aparece verde aunque la suite tenga rojos. Regla P11 no puede enforcer con este workflow. Canónico:

```yaml
run: |
  set -o pipefail
  npx playwright test ... 2>&1 | tee /tmp/salida.log
```

Excepción única: pipes donde `|| true` es intencional (ej. `--update-snapshots` que debe generar baselines aunque los tests fallen la primera vez). Ahí el `|| true` documenta la intención en el propio `run:` — no es el mismo caso.

Segundo: **cualquier bump de dependencia que cambie requisitos de runtime (versión de Node, versión mínima de navegador, WebSocket nativo, extensiones binarias) se verifica contra la versión efectiva del runtime en producción antes de mergear**. "Vercel usa el default" no basta — cada bump obliga a confirmar (a) la versión de Node del proyecto en Vercel Dashboard → Settings → Node.js Version, (b) grep del changelog del paquete por "requires Node X" o "requires WebSocket". Cuando la app no declara `engines` en `package.json` ni tiene `.nvmrc`, Vercel usa el default vigente al momento del deploy — que cambia sin aviso.

Incidente que originó la regla (2026-09-17 sprint bloque-h H-1 + vistas-doble): el H-1 lock regen llevó `@supabase/supabase-js` de 2.84.x → 2.109.0 dentro del rango `^2.84.0` del `package.json`. En 2.109 el `realtime-js` exige WebSocket nativo, que Node 20 no tiene. Consecuencia: `Error: Node.js 20 detected without native WebSocket support` en `fixtures/supabase.ts:79` → **19 tests F2 fallaron + 22 skipped en el PR #60**, pero el `tee` del step enmascaró el exit code → step "success" con conclusion=success → merge autónomo con CI verde falso. **Producción NO afectada** (Vercel default es Node 22, con WebSocket nativo — endpoint probe empírico `/api/servicios/{uuid}/slots` respondió HTTP 404 semántico tras `createClient` + query real). Solo CI (Node 20 pin) rompió, y el PO tuvo que pedir el probe explícito para descartar afectación prod.

Fix inmediato aplicado: (a) `set -o pipefail` en los 2 steps `| tee` del `e2e-error-audit.yml`, (b) pin `@supabase/supabase-js@~2.84.0` (patch-only, evita re-bump automático), (c) audit de PRs mergeados con F2 rota en silencio (ventana 2026-09-17: 4 PRs, solo 1 con código productivo — vistas-doble #60). Subir a Node 22 en CI queda como sprint separado (también cambia runtime de prod si eventualmente Vercel actualiza su default — se hace con calma). Regla escrita en el mismo commit del hotfix para que la próxima persona que agregue un `| tee` a un workflow lo vea antes de mergear.

**Specs `estado-actual` por grep sobre el código fuente afirman comportamiento, no comentarios — REGLA OPERATIVA (no P-numerada, 2026-09-17)**: los tests structural que graban un fix sobre el source con `readFile` + `expect(source).toMatch(pattern)` deben (a) excluir comentarios del match — un `pattern` que matchea un literal dentro de un JSDoc explicativo, un `//` inline, o el propio bloque de contexto que el sprint dejó como registro histórico, se rompe sin señal si alguien reformatea el comentario mañana; (b) preferir afirmar sobre el comportamiento observable (call site del helper que existe, prop pasada, componente renderizado) cuando sea posible, y solo caer al grep de literal cuando el ambiente structural no permite un smoke runtime real. Un test que se rompe por un JSDoc no protege nada — solo agrega ruido y presiona a que "el próximo refactor" borre el comentario para no romper el test, invirtiendo el flujo (el test dicta el comentario, no al revés).

Antídoto operativo al escribir un spec `estado-actual`: (1) si podés grep del call site real (`useX(...)`, `<Componente prop=`, `emit('evento')`, `fetch('/api/...')`), preferilo — es lo que un usuario del código consume; (2) si necesitás grep de literal, aplicalo sobre el source con comentarios stripped (`source.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^[ \t]*\/\/[^\n]*$/gm, '')`) antes del `.toMatch(pattern)`; (3) si el `pattern` es un identificador de sprint (`Sprint xxxxx-yyyy (2026-XX-XX)`) o un tag de fix (`// FIX-BUG-N`), tenés un test que auto-borra su propia utilidad al primer refactor de comentarios — reemplazalo por assertion sobre el efecto que ese sprint aterrizó.

Incidente que originó la regla (sprint tests-desactual PR #62, 2026-09-17): 8 tests structural del bloque C-D-E marcados `test.fixme` porque el pattern buscaba un literal que solo existía en un JSDoc o comentario `//` explicativo del sprint donde el fix aterrizó — el fix productivo YA estaba en prod, pero el comentario que citaba el literal fue reformateado en un refactor posterior y rompió el test. El fix del PR fue "strip comments before matching" — solución sintomática. La regla acá evita la reincidencia: no escribir specs que dependan del texto de un comentario para pasar.

**Nunca mergear con un check rojo — REGLA PERMANENTE (P11)**: incluso cuando el fallo parece ambiental (flake CI, timeout Supabase, cold preview, saturación cross-PR), **primero se arregla el ambiente, después se mergea**. Los flakes que se ignoran hoy son los que mañana esconden una regresión real — un check en rojo mergeado con "es flake" pierde la señal para siempre; el próximo rojo idéntico se lee igual y nadie mira si esta vez sí era bug. El costo operativo de "arreglar el ambiente" es una décima del costo de una regresión que entró silenciosa. Regla mecánica: **cero merge autónomo con un solo check no verde**; el auditor propone el fix del ambiente (concurrency, timeout justificado con incidente, retry policy, etc.) y espera GO del PO antes de aplicar. Incidente que originó la regla: PR #15 (L1-1 · sprint launch-l1, 2026-09-09), Playwright fail persistente en 4 reruns por `beforeAll` timeout en `f2-3/*` — causa: contención cross-PR sobre Supabase staging (múltiples PRs corrían en paralelo con concurrency por-ref). El auditor propuso 3 opciones (aumentar timeout / mergear con 3 verdes + 1 flake / workers=1) — el PO rechazó las 3 y aterrizó Opción D: `concurrency: { group: 'e2e-staging', cancel-in-progress: false }` a nivel workflow → serializa runs cross-PR contra staging → problema estructural resuelto en un commit + regla escrita para futuros sprints.
...
**No construir lógica de sesión sobre eventos del SDK cuya semántica no esté garantizada por contrato — REGLA OPERATIVA (no P-numerada, 2026-08-25)**: cuando la corrección funcional depende de "el SDK dispara evento X solo cuando pasa Y", si la relación X↔Y **no está garantizada por documentación oficial contract-level**, no la uses como pilar de lógica productiva. Los events de los SDK de terceros (`onAuthStateChange`, `onSnapshot`, `onmessage`, etc) suelen tener semántica más amplia que el nombre sugiere, y el proveedor puede cambiar comportamiento entre versiones minor sin ruido (patrón que llamamos anteriormente "assumption drift" del SDK). Si necesitás la correlación X↔Y para un feature, generá tu propia señal desde una acción **explícita e intencional del usuario** (submit de un form, click en un botón específico, POST exitoso a un endpoint) y expone un helper que se llama desde esos puntos. Cero dependencia del comportamiento del SDK que puede cambiar mañana sin aviso.

Antídoto operativo antes de escribir código que subscribe a un event del SDK:
1. Buscar la definición oficial del event en la docs del proveedor (no en un ejemplo de blog).
2. Enumerar TODOS los momentos donde el SDK puede disparar ese event (no solo los que "esperás").
3. Si algún momento no matchea la lógica que querés implementar, el event NO es la señal correcta — buscar una señal propia disparada por acción del user.

Incidente que originó la regla (sprint email-landing session-timeout fix, 2026-08-25):
`components/SessionTimeout.tsx` agregó subscribe `onAuthStateChange` para resetear el marker de inactividad en `SIGNED_IN`. Hipótesis: "solo login genuino dispara SIGNED_IN, re-hidratación es INITIAL_SESSION". La docs de Supabase v2.84 lista `INITIAL_SESSION` como event separado, entonces el filtro `event === 'SIGNED_IN'` parecía correcto. **Pero Supabase JS también dispara `SIGNED_IN` en cada F5 con sesión activa** (refresh silente al mount, comportamiento no documentado en superficie). Consecuencia: cada carga de página reseteaba el marker → **timeout de inactividad desactivado en toda la app** (silent kill de una capacidad de seguridad). Detectado por PO con smoke específico "positivo conocido" (marker viejo + F5 debe expulsar). Sin ese smoke habríamos aterrizado en prod con sesiones eternas.

**Fix estructural**: eliminar el subscribe, reemplazar por helper `resetInactivityTimer()` (en `lib/sessionTimeout.ts`) llamado desde 4 sitios de intención inequívoca del usuario:
- `pages/login.tsx` post-`signInWithPassword` success.
- `pages/register.tsx` post-signup wizard success.
- `pages/email-confirmado.tsx` cuando `user` cambia de null a poblado **Y** ` freshAuthTokenSeen === true` (doble-guard para no disparar por re-hidratación de sesión sin token en URL).
- `pages/completar-registro.tsx` post-POST success.

Cero dependencia de events del SDK, semántica honesta ("el user acaba de autenticarse activamente"), y la próxima versión del SDK puede cambiar sin afectar el guard.

**Meta-lección del sprint**: dos fixes consecutivos del mismo sprint cayeron en el mismo antipatrón — el original `sessionReady` que dependía del `SIGNED_IN` listener local (loader pegado), y el `fix 2` que dependía del `SIGNED_IN` como marker de intención (timeout desactivado). Ambos asumían semántica no verificada del mismo event. La regla queda como red para futuros sprints — cualquier PR que introduzca `onAuthStateChange` subscribe con lógica condicional por event debe justificar por qué esa condición es semánticamente estable, o cambiar a señal propia.

**Una pantalla de estado no debe afirmar una causa que no verificó — REGLA OPERATIVA (no P-numerada, 2026-08-25, patrón del día con 3 instancias)**: los mensajes de error/tránsito en la UI deben **describir el efecto observable** ("Este enlace ya no sirve", "Cerramos tu sesión") **no la causa inferida** ("expiró", "por inactividad") — a menos que la causa esté empíricamente confirmada en el path exacto que llevó a la pantalla. Cuando un mismo destino cubre múltiples causas (destino compartido entre `path_A_verified_cause` + `catch fallback with unknown cause`), el copy debe ser causa-neutral. Afirmar una causa específica que no aplica al 100% de los caminos que aterrizan ahí es misinformación al usuario en el momento exacto donde necesita orientación clara — mismo antipatrón que la sala de espera post-F1 que decía "tu solicitud está en revisión" cuando nadie estaba revisando.

Es el corolario P8 11ª aplicado al copy de UI: no afirmar sin verificar. Cuando el auditor está por escribir una frase del tipo "por X razón hicimos Y", contrastar la razón contra los paths reales que aterrizan en esa pantalla. Si algún path aterriza ahí por otra razón, el copy es misinformación en ese subset — reescribir causa-neutral o separar pantallas.

Antídoto operativo antes de escribir copy de error/tránsito: (a) mapear todos los paths que aterrizan en la pantalla (grep `router.push('/path')` + `router.replace('/path')` + `window.location.href = "/path"` en el codebase); (b) para cada path, escribir la causa real que dispara; (c) si hay > 1 causa distinta, escribir el copy sobre el EFECTO observable común, no sobre las causas específicas; (d) si querés preservar una causa específica en algún path, separar el destino en 2 pantallas distintas o pasar un query param que discrimine.

**3 instancias del sprint email-landing 2026-08-25** (mismo día, mismo antipatrón aplicado a 3 pantallas distintas):

- **"El enlace expiró"** cuando el link estaba consumido (no vencido). Supabase Auth devuelve `error_code=otp_expired` para AMBOS casos — misinformación afirmaba "duran un tiempo limitado, regístrate de nuevo" a users que solo debían hacer login. Fix: colapsar a "Este enlace ya no sirve" + copy causa-neutral que menciona ambas posibilidades sin afirmar cuál pasó (commit 0fa0ead).
- **"Procesando token..."** cuando el token ya estaba procesado hacía segundos. Loader pegado indefinido porque `sessionReady` state esperaba un event que se perdió en race. UI comunicaba "trabajando" cuando el sistema ya había terminado. Fix: eliminar `sessionReady`, derivar de `user` del UserContext, kill-switch defensivo 4s con copy que AFIRMA lo que el sistema garantiza (email_confirmed_at server-side pre-render) (commit c8296a1).
- **"Periodo de inactividad"** cuando pasaron 29 segundos post-signup. Guard `SessionTimeout` catch incondicional redirigía a `/security-logout` ante cualquier throw de `getSession()` con copy que afirmaba inactividad — cero verificación de la causa real. Fix: catch no-expulsivo (log Sentry + return silente), exclusión de rutas de auth transit, subscribe SIGNED_IN para resetear marker, + copy `/security-logout` reescrito causa-neutral "Cerramos tu sesión / Por seguridad cerramos tu sesión. Vuelve a entrar para continuar." (commit del sprint session-timeout fix).

En los 3 casos: el sistema funcionó correcto por dentro, la pantalla mintió sobre lo que pasó, el user tomó decisiones sobre información errónea. El patrón cuesta abandono real de usuarios (7 legacy en pantalla "sala de espera"; 15 huérfanos "nunca confirmaron" que quizás recibieron loader pegado o error engañoso). Regla aterrizada aquí como caso canónico para evitar la 4ª instancia.

**Pantallas de tránsito async llevan kill-switch temporal con CTA manual — REGLA OPERATIVA (no P-numerada, 2026-08-25)**: cualquier pantalla que depende de un evento asíncrono externo (SIGNED_IN de Supabase Auth, respuesta de un endpoint, hidratación de un context, notificación de un servicio de terceros) para transicionar a su estado siguiente debe tener **red de seguridad temporal** — un `setTimeout` (~4-5s) que fuerza un estado terminal alternativo con acción manual si el evento no llega. Sin kill-switch, cualquier fallo del evento (race donde se disparó antes del suscribe, listener perdido por remount, SDK que consume silente sin señal) deja al usuario en un limbo indefinido — el patrón "producto funcionando por dentro, comunicando lo contrario". El fix de la causa raíz elimina el race conocido; el kill-switch garantiza que el próximo race desconocido no vuelva a colgar la UI. Es defensa en profundidad, no reemplazo del fix.
...
// Render:
// isWaitingForEvent && timedOut → <FallbackScreen>  (heading que AFIRMA
//   el efecto que ya sucedió server-side + CTA manual "Ir a login" o
//   equivalente. NO usar "debería" ni "puede que" — afirmar con datos
//   verificados server-side, no con hipótesis sobre el estado UI).
// isWaitingForEvent → <LoadingScreen>
```

**Regla de copy del fallback**: el mensaje del fallback debe **afirmar** el efecto que ya sucedió, no dudar. Si Supabase Auth valida el token server-side antes de redirigir a la landing (email_confirmed_at poblado), el copy dice "Tu cuenta ya está activa", no "Tu cuenta debería estar confirmada". El "debería" transmite duda al usuario justo cuando necesita certeza — y el estado real es el de éxito, no de incertidumbre. Verificar contra la BD si tenés dudas de qué está garantizado por el sistema antes de decidir el copy — no escribir hipótesis defensivas cuando hay dato empírico disponible.

**Regla de verificación del kill-switch**: nunca dejar un `setTimeout` en producción sin **probar empíricamente** que dispara — el patrón P8 aplicado a UI. Método canónico para bloquear la señal esperada y forzar el timeout: Chrome DevTools → Network → `Request blocking` con pattern del host que provee la señal (ej. `*.supabase.co/*` para bloquear Auth REST). Signup fresco, click al link, contar los 4s exactos, verificar que aparece el fallback. Sin ese smoke inducido, el kill-switch es acto de fe — puede reportar OK sin funcionar y nadie se entera hasta que la causa raíz vuelva.

Incidente que originó la regla: sprint email-landing loader fix (2026-08-25). `pages/email-confirmado.tsx` tenía `useState sessionReady` que dependía de un `SIGNED_IN` event capturado por listener local — el SDK global de Supabase disparaba el evento ANTES del mount del componente + suscribe del listener. Race perdido → `sessionReady` quedaba en `false` indefinido → loader pegado 60+ segundos hasta hard refresh. El fix causa raíz eliminó `sessionReady` completo (usar `user` del UserContext como señal), pero el kill-switch defensivo (4s → fallback "Tu cuenta ya está activa") aterrizó en el mismo commit para cubrir cualquier otro race que emerja a futuro (SDK version bump, nueva ruta OAuth, cache stale de service worker). Copy verificado: `SELECT email_confirmed_at FROM auth.users WHERE email='<smoke>'` — poblado siempre pre-render, entonces afirmar es correcto empírico, no hipótesis.
...
**Antes de clasificar un issue de Sentry (o cualquier telemetría de usuario), cruzar el `user.id` — y cuando el id falta, el navegador/dispositivo — contra las cuentas del PO y las cuentas de prueba conocidas — COROLARIO P8 (adición 2026-09-25, 12ª instancia del meta-patrón — generalización de "verificar contra positivo conocido" al ORIGEN de la evidencia)**: los corolarios previos verifican que el mecanismo de medición funcione (10ª) y que la atribución causal esté sostenida por los datos (11ª). Este corolario cierra el eslabón anterior: **verificar quién generó la evidencia antes de clasificarla como señal del sistema**. Un cluster de events en Sentry (o hits en GA4, o filas en un dashboard de errores) puede estar 100 % explicado por el propio equipo probando el sistema — el auditor, el PO, las cuentas E2E de la suite Playwright. Si el auditor lee ese cluster como "usuarios reales afectados" y clasifica el issue como BLOQUEA/CONVIENE productivo, gasta sprint en un problema que puede no existir en la base de usuarios reales, y peor, la clasificación se aterriza a docs (BACKLOG.md, actas) como hecho establecido.

**Aclaración operativa ampliada (adición 2026-09-25, mismo turno)** — la regla "cero código productivo de prueba en producción" **aplica también a páginas y rutas gateadas por entorno**. Una página `pages/foo.tsx` con `getServerSideProps` que responde `notFound:true` cuando `NEXT_PUBLIC_VERCEL_ENV === 'production'` sigue siendo código desplegable en el bundle productivo: la ruta compila, el archivo se sirve, y el gate depende de una env var que puede mal-setearse (rotación de config, staging con la env de prod, deploy accidental sin la env var → ruta activa donde no debería). Además invita a añadir superficie que solo el equipo consume — antipatrón para un producto de terceros. **La verificación en staging (o en el pipeline) se hace con specs, fixtures o tests unitarios**, nunca con código desplegable condicional. Cuando el patrón que se busca verificar es "el componente productivo hace X cuando ocurre Y", el test correcto es unitario: mockear las dependencias externas del componente (SDK, router, contexto) y llamar al método directo (`componentDidCatch`, callback, handler), no montar una ruta que dispare Y en runtime real. Cuando el patrón es "el usuario ve Z al hacer W" y Z ya ocurre en producción con un bug real conocido, la verificación end-to-end es reproducir el bug real tras el fix (positivo natural), no fabricar un throw sintético en una ruta gated. **Incidente que originó la aclaración (mismo sprint sentry-boundary 2026-09-25)**: la primera propuesta del auditor para verificar el fix del `componentDidCatch → Sentry.captureException` fue `pages/staging/error-boundary-smoke.tsx` con `throw new Error(...)` gated por `NEXT_PUBLIC_VERCEL_ENV`. El PO detectó la contradicción con la regla existente antes del GO y forzó la corrección: eliminar la ruta + spec e2e, reemplazar por [components/ErrorBoundary.test.tsx](components/ErrorBoundary.test.tsx) (patrón g1-*, mock `@sentry/nextjs` vía `require.cache`, corre bajo `ci.yml > typecheck-and-build`). El positivo real en prod queda ligado al flujo del incidente (`chat → back → /usuario`) que el PO reproduce post-merge — bug real en el codebase, cero throw sintético desplegado. **Regla operativa**: antes de aterrizar la clasificación de un issue Sentry en un acta o entrada de BACKLOG, ejecutar el cruce contra los `user.id` conocidos del equipo. En este proyecto: `aff2a90d…` = PO desktop Windows Chrome, `0c2ab509…` = PO Android Chrome, más las cuentas Playwright (`E2E_STAGING_EMAIL`, `E2E_STAGING_TUTOR_EMAIL`, cuentas efímeras del BELL-150 stress con prefix `bell-150-stress-*`). Cuando el `user.id` de un event coincide con uno del equipo → **NO** es "usuario real"; es evidencia de dev/QA. Cuando el issue tiene `userCount=0` (Sentry sin `setUser`), usar la señal secundaria disponible: browser/device fingerprint (`Chrome Mobile 152 Android 15` = mobile del PO) + horario (fuera del rango típico de usuarios reales en Chile) + ruta (rutas admin/staging = típicamente equipo). Si el cruce da inconcluso, marcar la clasificación como "hipótesis, sin verificar" hasta poder identificar el origen. **Incidente que originó el corolario**: sprint prelaunch CUE-1 (2026-09-15) + cue-1-fix (2026-09-22) + sentry-boundary (2026-09-25). Durante dos semanas el issue `JAVASCRIPT-NEXTJS-7` (`user_context_stuck`) fue clasificado en BACKLOG como "CUE-1 BLOQUEA · 32 events reales en 7 días" — el sprint `cue-1-sentry-user` (#82, 2026-09-24) puso `Sentry.setUser({id})` para poder atribuir, y al cruzar los `user.id` post-#82 el único que aparecía era `aff2a90d…` = el propio PO probándose desde desktop. Los 5 events adicionales del cluster hydrate exhausted / login_role_lookup_failed (issues -5/-8) resultaron también del PO desde `0c2ab509…` = su móvil Android con red intermitente. Cero usuarios reales afectados en 28 días de monitoreo. Costo del error: dos semanas de sprint activo por evidencia que era el operador probándose a sí mismo — el fix estructural del watchdog (PR #85) resultó ser el correcto pero por la razón equivocada (fix real del stale closure, no del "cuelgue estructural en usuarios reales"), y F1+F2 (AbortController + fallback) se ejecutaron hasta B2 antes de descartarse como "no aplican" cuando el análisis de user.id llegó tarde. **Antídoto operativo**: en el kickoff de cualquier sprint motivado por telemetría Sentry, la primera línea del reporte al PO es "issue X tiene N events; cruce de `user.id` contra cuentas conocidas: [X del PO desktop / Y del PO móvil / Z anónimo / A cuenta de prueba / B usuario real desconocido]"; **la clasificación (BLOQUEA/CONVIENE/monitoreado) NO se emite antes de ese cruce**. Si el cruce no se puede hacer (por `userCount=0` sin `Sentry.setUser`), el primer entregable del sprint es agregar `setUser` — el sprint pausa a la espera de datos atribuibles. Aplica a Sentry, GA4 (client_id vs cuentas de prueba), Vercel Analytics, cualquier dashboard donde un cluster de eventos disparen un sprint.
...
**~~SQL Editor de Supabase NO mantiene transacciones entre ejecuciones separadas — COROLARIO P8 (adición 2026-08-14 noche, 6ª instancia del meta-patrón)~~** — **AFIRMACIÓN RETIRADA 2026-09-01 (sprint default-privs)**. El texto original se preserva en el bloque siguiente como registro histórico, pero **no debe citarse como regla del proyecto**.

- **Fecha original**: 2026-08-14 noche, agregada tras 1 incidente aislado (A2 prod, cleanup `es_ejemplo=true`).
- **Retracción verbal previa**: 2026-08-27, sprint `admin-visibilidad` — el PO observó que separa corridas rutinariamente sin ver ROLLBACK silente. La retracción verbal no aterrizó al documento; el corolario siguió vivo en CLAUDE.md hasta que reapareció como fundamento de diseño en el sprint `default-privs` ronda 2 (2026-09-01), donde el PO forzó la corrección.
- **Sustento ausente**: 1 solo incidente registrado, sin verificación repetida ni contraste con workflow habitual. La observación empírica del PO (múltiples separaciones exitosas de corridas sin rollback silente) **nunca se refutó con datos**.
- **Instrucción al lector futuro**: si necesitás saber cómo se comporta el SQL Editor de Supabase con corridas separadas, **medilo** en tu proyecto/versión con un caso simple (`BEGIN;` + `SELECT 1;` en corrida 1 + query de verificación en corrida 2 — chequear si la transacción de la primera persiste). NO citar este bloque como regla. Ver también entrada BACKLOG "Auditoría de los corolarios P8 de CLAUDE.md" (sprint default-privs 2026-09-01, prioridad media).
- **Texto original retractado (preservado para no borrar historial)**: "cada botón 'Run' abre su propia transacción implícita y hace COMMIT/ROLLBACK al terminar la corrida. Un `BEGIN;` en una corrida y un `COMMIT;` en otra NO forman una única transacción — el `BEGIN;` de la primera corrida cierra en ROLLBACK al finalizar esa ejecución, y el `COMMIT;` de la segunda no tiene transacción abierta que persistir. `RETURNING` de un `DELETE` en la primera corrida devuelve las filas correctamente (parece exitoso) pero la transacción hace ROLLBACK silencioso al terminar la corrida. Regla operativa: `BEGIN;` + `DDL/DML` + `COMMIT;` deben ir en un único bloque enviado con un único click de Run. Nunca separar corridas."
...
**El callback de `onAuthStateChange` corre DENTRO del lock de auth — REGLA PERMANENTE (P10)**: nunca hacer llamadas asíncronas al cliente Supabase dentro del callback de `onAuthStateChange`. El SDK ejecuta ese callback mientras tiene tomado su lock interno de auth. Cualquier query que necesite el access token — que son todas las de datos — va a intentar re-adquirir ese mismo lock, quedar encolada, y esperar a que termine el callback que está esperando a la query. Deadlock circular. No lanza excepción, no falla la request, no aparece nada en consola: la UI queda cargando para siempre.

Si hay que hacer trabajo asíncrono en respuesta a un evento de auth, diferirlo fuera del lock:
```js
setTimeout(() => { if (!mounted) return; hacerElTrabajo(session); }, 0);
```

El `setTimeout` es lo que rompe el ciclo. Un guard de identidad que evite re-ejecutar cuando el usuario no cambió es útil para ahorrar trabajo, pero **NO protege del deadlock**: es optimización, no arreglo. Si se saca el diferido dejando el guard, el bug vuelve por cualquier otro camino.

**Dos señales de que estás frente a este problema**:
- El mismo código funciona en una invocación y cuelga en otra. Revisá si una corre después del lock (por ejemplo vía `getSession().then(...)`) y la otra dentro del callback.
- Cuelgan **TODAS** las queries, no una. Si fuera RLS o una tabla puntual, colgaría esa sola.

**Reemplazar la implementación del lock no alcanza**. El SDK mantiene su propio estado de reentrada (`lockAcquired` / `pendingInLock`) independiente del lock que se le pase, así que un lock no-op evita otros problemas (Web Locks huérfanos) pero deja el deadlock intacto.

Esta regla es de la misma familia que la que ya teníamos por SessionTimeout (2026-08-25): **no construir lógica sobre eventos del SDK cuya semántica no esté garantizada por contrato**. Acá el agravante es que el evento `SIGNED_IN` se dispara también para sesiones ya existentes, no solo en logins genuinos.

**Caso de origen**: sprint `deadlock-fix`, 2026-08-28. Ver [ACTA_DEADLOCK_FIX.md](ACTA_DEADLOCK_FIX.md) para el diagnóstico completo (6 hipótesis, stack trace verificado contra `node_modules/@supabase/auth-js/dist/main/GoTrueClient.js` v2.84.0, y los 3 tests de aceptación con números literales — T1a 339 ms, T1b 102 ms vs control negativo 20 000 ms, T3 header Admin verde).

**Ningún push directo a main, ni docs-only — REGLA OPERATIVA (no P-numerada, 2026-09-22)**: todo cambio a `main` entra por Pull Request con checks verdes. **Cero excepción por "es solo docs" / "es sólo config del CI" / "es 1 línea"**. Cada push a `main` dispara deploy automático de prod en Vercel (aunque sea `.md` — el build de Next corre igual y sirve un SHA nuevo); el flujo PR + checks es la única puerta con evidencia auditable de que el cambio pasó por gate. Durante el **congelamiento de prod (Tramo 2 del plan-lanzamiento-nov-2026, 2026-09-29 al 2026-10-27)** esta regla incluye docs — cero excepción. **Incidente que originó la regla**: commit `f0955d3` (2026-09-22) — auditor pusheó docs-only directo a main (cierre F2-3-CLEANUP + regla operativa de verificaciones) saltándose el flujo PR. El PO detectó el desvío en el turno siguiente y confirmó dos consecuencias: (1) disparó deploy de prod en Vercel sin gate; (2) rompió el patrón PR-only. **Refuerzo estructural**: la protección de rama `main` en GitHub debe bloquear direct push (ver "Ajuste de branch protection" abajo) — sin ese candado a nivel plataforma, la regla depende solo de disciplina del auditor y falla en el primer despiste. Antídoto operativo mientras el candado no está: **antes de cualquier `git push` con `main` como upstream, verificar `git branch --show-current` NO devuelve `main`** — si devuelve `main`, abortar y crear rama nueva (`git checkout -b <slug>`) para PR.

**Ajuste de branch protection (para aplicar por PO en `Settings → Branches → Add branch ruleset` o `classic branch protection rule` sobre `main`)**:
...
- **Require status checks to pass** ✅ — habilitar y agregar como required:
  - `typecheck-and-build`
...
- **Restrict deletions** ✅ (evita `git push origin --delete main`).
- **Restrict pushes** ✅ o equivalente que **bloquee direct push a main desde cualquier actor** — solo aceptar PR merges. En GitHub Rulesets moderno esto es "Restrict pushes"; en classic branch protection es "Restrict who can push to matching branches" + lista vacía.
- Bypass: dejar en "Nadie" o solo PO como bypass explícito para emergencias (hotfix Sentry — ver excepción del Tramo 2). El auditor NUNCA en bypass.
...
**El reporte de cierre responde punto por punto las verificaciones pedidas antes del merge — REGLA OPERATIVA (no P-numerada, 2026-09-22)**: cuando el PO pide verificaciones específicas antes de mergear (formato "confirma que 1) ..., 2) ..., 3) ...", "control negativo antes/después", "cita el ref del proyecto", etc.), el reporte de cierre del sprint **responde cada punto textualmente, en el mismo orden, o dice explícitamente por qué no puede responderlo**. Un reporte de cierre que omite ítems pedidos **no cierra el sprint** — el PO tiene que volver a pedirlos, se pierde el rastro auditable de la evidencia entregada, y se erosiona la confianza en el criterio "verde con verificación" (que es el que autoriza el merge en primer lugar). Antídoto operativo: antes de dar por cerrado un sprint que llegó con verificaciones pedidas, releer el turno donde el PO las enumeró y **construir la respuesta como lista numerada mirror del pedido**. Si un ítem del pedido no se puede responder (dato no capturado, verificación no ejecutada), decirlo explícitamente — cero relleno "de memoria" ni reconstrucción inferida. Incidente que originó la regla: cierre F2-3-CLEANUP (2026-09-22 PR #78), el PO había pedido 4 verificaciones antes del merge (ref del proyecto, predicado exacto, control negativo antes/después, clasificación de residuos restantes). El auditor entregó las respuestas en un turno intermedio antes del merge, pero el **mensaje de cierre post-merge omitió los 4 puntos** — asumió que "ya estaban dichos" antes. El PO tuvo que pedir explícitamente que se aterrizaran en el acta del sprint, marcando la regla para futuros cierres. La aplicación en cierres posteriores es escritura en la propia acta del sprint (`docs/sprints/<sprint>.md`), no solo en el mensaje del chat — el chat se pierde, la acta queda.
...
**Checklist obligatorio al crear rama nueva — ANTES del primer `git push`**:
1. `echo -n "<nombre-propuesto>" | wc -c` (o contar mental si es evidente ≤ 12 chars). Presupuesto: **≤ 18 chars**.
2. Si excede 18: renombrar la rama local ANTES de pushear (`git branch -m <nombre-propuesto> <nombre-corto>`) — cero commits, cero PR, cero CI-rojo por DNS truncation. Bajar el costo del incidente a segundos.
3. Si ya se pusheó y CI dio rojo por `ERR_NAME_NOT_RESOLVED` en el setup Playwright, aplicar el flow de rename operativo: crear rama corta desde el head actual, push, PR nuevo con misma descripción + enlace al anterior, cerrar PR viejo sin merge con comentario que apunte al nuevo. Ver PR #89 → #91 (2026-09-24) como caso canónico: rama `incidente-usuario-fix` (21 chars) reventó el CI, rename a `hf-usuario-fix` (14 chars) resolvió sin tocar código.

Incidentes registrados de esta violación: (a) `sentry-1-hotfix-csp` 2026-08-11 (19 chars, 8 min polling). (b) `incidente-usuario-fix` 2026-09-24 (21 chars, 2 failed + 143 did not run del setup Playwright, PR #89 cerrado y reabierto como #91). La segunda instancia motivó agregar el checklist obligatorio de conteo pre-push como paso operativo formal — la regla estaba escrita explícita y se ignoró.
