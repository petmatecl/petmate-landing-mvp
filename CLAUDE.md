...
- **Antes de cualquier fixme, verificar si el spec pasa en los últimos runs verdes** (`gh run list --workflow <name> --status success --limit 5 --json databaseId,headBranch,headSha,createdAt`). Si el test aparece verde en runs recientes de PRs no relacionados → es flaky por ambiente (concurrencia cross-PR, saturación Supabase, race timing), no bug estructural del test o del código productivo. La evidencia va al acta como run_ids concretos + branches donde pasó. Si el test **también falla** en algún run verde reciente (por definición no fue "verde" — hay que revisar `--status failure`), la hipótesis de flake se debilita → escalar diagnóstico más profundo antes del fixme.
